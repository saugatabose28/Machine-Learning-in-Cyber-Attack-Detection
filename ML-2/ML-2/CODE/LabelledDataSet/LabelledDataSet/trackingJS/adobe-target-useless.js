...
(function() {
    ...
    _AT.hideImageStyleElementId = "at-hide-all-images";
    _AT.actions = [];
    var admin=~window.location.search.indexOf("_AT_Admin=1")||~document.referrer.indexOf("_AT_Admin=1");
    ...
    if (!admin && typeof mboxFactoryDefault !== "undefined") {
        _AT.isRedirectedPage = mboxFactoryDefault.getCookieManager().getCookie("at-redirect-offer") === "true";
                        ...
                        if (type === "setAttribute") {
                            ...
                            if (actionIsToChangeImageSrc(action)) {
                                if (action.height && action.width) {
                                    elem.style.height = action.height + "px";
                                    elem.style.width = action.width + "px"
                                }
                            ...
                            if (type === "setStyle") {
                                ...
                                if (type === "trackSubmit") {
                                    ...
                                    if (type === "trackClick") {
                                        ...
                                        addClickTracker(elem, action.selector)
                                    } else {
                                        if (type === "rearrange") {
                                            ...
                                            if (type === "resize") {
                                                ...
                                                if (type === "move") {
                                                    ...
                                                    if (type === "insertAfter") {
                                                        log("Inserting after ", getFragment(action.content), "to element ", elem);
                                                        insertAfter(action, elem)
                                                    } else {
                                                        if (type === "prependContent") {
                                                            ...
                                                            if (type === "redirect") {
                                                                if (!_AT.isRedirectedPage && typeof mboxFactoryDefault !== "undefined") {
                                                                    mboxFactoryDefault.getCookieManager().setCookie("at-redirect-offer", "true", 30);
    ...
    var prepareBeforeApplyingActions = function(actions) {
        ...
        if (imagesPreloadedCount > 0&&!document.getElementById(_AT.hideImageStyleElementId)) {
            addStylesToHideImages()
        }
    };
    ...
    var applyRemainingActionsWithCounter = function(actions) {
        var counter = 0, applyRemainingActionsInterval, logErrorConfig = {
            ...
            if (actions.length && counter < _AT.config.tryAfterLoadMaxCount) {
                ...
                if (typeof _AT.removeAllImagesStyle == "function") {
                    _AT.removeAllImagesStyle()
                }
    ...
    var addClickTracker = function(elem, selector) {
        var handler = function() {
            removeListener(elem, "click", handler);
            trackClick(selector, ["target-click-url=" + selector])
        };
    ...
    var insertAfter = function(action, elem) {
        var content = action.content, wrapperElement = document.createElement("div");
        if (typeof action.asset !== "undefined") {
            content = '<img src="' + action.value + '"></img>'
        }
        elem.parentNode.insertBefore(getFragment(content), getNextElementSibling(elem));
        wrapperElement.appendChild(getFragment(content));
        executeScript(wrapperElement)
    };
    var redirectPage = function(action, elem) {
        var searchString = window.location.search, queryString = "", redirectUrl;
        ...
        redirectUrl = action.url + queryString + (action.passMboxSession ? "mboxSession=" + mboxFactoryDefault.getSessionId().getId() : "");
    ...
    var addStylesToHideImages = function() {
        var hideAllImagesStyleTag = appendStyle("img {visibility:hidden}", _AT.hideImageStyleElementId), removeAllImagesStyleTimeOutId;
        _AT.removeAllImagesStyle = function() {
            var hideAllImagesStyleElement = document.getElementById(_AT.hideImageStyleElementId);
            removeElement(hideAllImagesStyleElement);
            clearTimeout(removeAllImagesStyleTimeOutId)
        };
        onReady(function() {
            removeAllImagesStyleTimeOutId = setTimeout(_AT.removeAllImagesStyle, _AT.config.fallbackTimeOut)
        })
    };
    ...
    if (admin) {
        if (typeof mboxCreate == "function") {
            var mboxToLoad = 0, __mboxCreate, target;
            __mboxCreate = mboxCreate;
            mboxCreate = function(name) {
                var mbox = __mboxCreate(name), mboxParams = Array.prototype.slice.call(arguments, 1);
                if (mbox) {
                ...
                while (target.nodeType !== 1 || target.tagName === "SCRIPT" || target.id.indexOf("mboxMarker") === 0);
                target.setAttribute("data-mboxName", name);
                target.setAttribute("data-mboxParams", mboxParams.length > 0 ? (_AT.JSON || JSON).stringify(mboxParams) : "");
