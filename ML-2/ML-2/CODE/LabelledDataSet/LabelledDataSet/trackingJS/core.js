...
function commonShowModalDialog(url, features, callback, args)
{
    ...
    } else
    {
        ...
        if (!features) 
            features = "width=" + defaultWidth + ",height=" + defaultHeight;
        else
        {
            function assocArray() {
                return new Array();
            ...
            if (!feats["width"]) 
                assocArray_add(feats, "width", feats["dialogwidth"] || defaultWidth);
            if (!feats["height"]) 
                assocArray_add(feats, "height", feats["dialogheight"] || defaultHeight);
            if (!feats["scrollbars"]) 
                assocArray_add(feats, "scrollbars", feats["scroll"] || defaultScrollbars);
            features = '';
...
function EEReplace(clientID, key, id)
{
    ...
    if (downlevel != null && autoPostBackEnabled(downlevel)) 
        schedulePostBack();
}
function EERemove()
{
...
function ConvertEntityToSpan(ctx, entity)
{
    ...
    if (extraData)
    {
        ...
        if (!data) 
            data = extraData.innerXml || extraData.innerHTML;
        if (!data) 
            data = '';
...
function saveOldEntities(elementId)
{
    ...
    if (elt != null && autoPostBackEnabled(elt)) 
        elt.oldEntities = getEntityKeysFromElement(elementId);
}
...
{
    if (m&&!MenuHtc_isOpen(m)) 
        MenuHtc_show(m, r, fr, ft, yoff);
}
function OMenuEvnt()
{
...
function MenuHtcInternal_Show(oMaster, oParent, y, fFlipTop)
{
    ...
    var fTopLevel;
    var oInnerDiv;
    if (!oMaster._oContents) 
        PrepContents(oMaster);
    if (!oMaster._oContents || IsOpen(oMaster)) 
        return;
    ...
    if (!oPopup)
    {
        ...
        if (browseris.nav)
        {
            oPopup.onkeypress = function(e) {
                return false;
            };
            oPopup.onkeyup = function(e) {
                return false;
            };
            ...
            oPopup.onmouseover = function(e) {
                PopupMouseOver(e);
                return false;
            };
            oPopup.onmouseout = function(e) {
                PopupMouseLeave(e);
                return false;
            };
            ...
            oParent.onmouseover = function (e) {
                PopupMouseOverParent(e);
                return false;
            };
            oParent.onmouseout = function(e) {
            ...
            oParent.oncontextmenu = function(e) {
                TrapMenuClick(e);
                return false;
            };
        } else
        ...
        if (fTopLevel)
        {
            ...
            if (wzOnUnload) 
                oPopup.onunload = new Function(wzOnUnload);
        }
    } else
    {
...
function ToggleMenuItem(oMaster, nLevel, oItem)
{
    ...
    if (!oItem || (oOld && oItem.id == oOld.id)) 
        return;
    if (oOld)
    {
        UnselectItem(oOld);
