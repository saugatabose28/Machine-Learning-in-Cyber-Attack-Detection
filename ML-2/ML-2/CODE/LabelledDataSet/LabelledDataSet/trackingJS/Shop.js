...
function SetsCookie(c_name,value,expiredays)
{
  ...
  ((expiredays==null) ? "" : ";expires="+exdate.toGMTString()+";path=/;domain=.petmountain.com");
}
...
QuickCart_checkQuckCart = function()
{
    ...
    if(QuickCart_div_cart_area_added) {
        if(QuickCart_jsonp_enabled){
            quck_cart_update_div = QuickCart_div_cart_status;
            QuickCart_JsonpRequest("http://www.petmountain.com/Shop/Cart/Request/Total?callback=QuickCart_JsonpResponseTotal");
        }else{
  ...
  var urlr = ajaxFormatUrl("Shop/Customer/MessageRequest");
    $j.get(urlr,function(data) {
      if(data != "") {
		...
		$j('.header_nav_log_in_sign_up').addClass('tc_ghost');
$j('#customer_message').addClass('ready').appendTo('.menu_header1');      }
    });
}
...
saveUserData = function() {
  $j.cookie('userData', serializeParams(userData), {expires:14,path:'/',domain:".petmountain.com"});
};
