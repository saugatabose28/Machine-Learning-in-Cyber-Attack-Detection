...
GDTDefine('gdt:mod/stat.js', function(require, exports, module) {
    ...
    StatMod.sendErrMsg = function(e, url, pid, opt) {
        var data, browserinfo, map, callerinfo = '', tmp, msg = e.message, stack = e.stack, extra, hasOrder, orderdesc = '', charleft = 8;
        opt = opt || {};
        extra = opt.extra;
    ...
    StatMod._postdata = function(url, data) {
        ...
        if (comm.fp && comm.fp.QZFL && (FormSender = comm.fp.QZFL.FormSender)) {
            ...
            emsgnum++;
            var frameId = '_GDT_POST_SENDER_' + emsgnum, cntid = "_GDT_SENDER_" + emsgnum, doc = document, wrapFrm, frameEl, formEl, df, tmp, elist, loaded = false;
    ...
    StatMod.sendStat = function(errcode, startTime, calledlist, opts) {
        var duration, clen = calledlist.length, isKP = false, statId, commStated = false, commStatId = 400350;
        opts = opts || {};
        ...
        function _send(pid) {
            var ecode = 0, succode = 1, url, urlext, rate = 1, errstatus;
            ecode = errcode;
    ...
    (function(q) {
        ...
        function valueStat(domain, cgi, type, code, time, rate, exts) {
            var param = [], uin = comm.cookie.get('uin');
            if (uin) {
            ...
            if (i > 0) {
                helper.pingreq(commurl + '?' + param.join('&'), 1000);
            }
        }
    ...
    StatMod._spnum = 0;
    StatMod._stimeout = 10000;
    StatMod.safeping = function(url) {
        StatMod._spnum++;
        var pingurl = 'window.name=frameElement.url;window.location.href="http://qzonestyle.gtimg.cn/qzone/biz/gdt/comm/viewproxy.html";';
        var dm = (document.domain == location.host) ? '': 'document.domain="' + document.domain + '";';
        html = '<html><head><meta http-equiv="Content-type" content="text/html; charset=UTF-8" /><\/head><body><script>' + dm + ";setTimeout(function(){" + pingurl + '}, 20);<\/script></body></html>';
        var ifrurl = 'javascript:document.open();' + dm + ';document.write(frameElement.html);document.close();';
        var frm = document.createElement("iframe");
        frm.url = StatMod._spnum + "|||" + url;
        frm.html = html;
        ...
        (function(frm, curnum) {
            if (window.postMessage) {
                ...
                var clear = function() {
                    frm.clear();
                    comm.event.removeEvent(window, 'message', cmes);
                    clearTimeout(timer);
                    timer = null;
                };
                cmes = function(event) {
                    var data = event.data;
                    if (event.origin !== 'http://qzonestyle.gtimg.cn')
                        return;
                    if (data && data.type == 'viewproxy') {
                        clear();
                    }
                };
                comm.event.addEvent(window, 'message', cmes);
                timer = setTimeout(clear, StatMod._stimeout);
            } else {
                var nav = window.navigator, cbname = 'viewproxycallback' + curnum, timer;
                var clear = function() {
                    nav[cbname] = null;
                    frm.clear();
                    clearTimeout(timer);
