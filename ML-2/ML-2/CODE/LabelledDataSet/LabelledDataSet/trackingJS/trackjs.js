...
PostAssoc = function() {};
PostAffAction = function(actionCode) {
...
PostAffAction.prototype._correctString = function(value, regexp) {
    ...
    strValue = this._removeDotButFirst(strValue);
...
PostAffAction.prototype._removeDotButFirst = function(source) {
    pos = source.indexOf('.');
    return source.substring(0, pos + 1) + source.substring(pos + 1).replace(/\./gi, '');
}
...
PostAffAction.prototype.setFixedCost = function(value) {
    this.f = this._correctCurrency(value);
};
...
PostAffAction.prototype.setCustomCommission = function(value) {
    this.cc = this._correctCommission(value);
};
...
PostAffAttributeWriter = function(idIn, attributeNameIn, urlParamNameIn, separatorIn) {
    var id = idIn;
    ...
    if (typeof urlParamName == 'string' && urlParamName != '') {
        value = new PostUrlReplacer(urlParamName, separator);
    } else {
...
PostUrlReplacer = function(urlParameterNameIn, separatorIn) {
...
PostValueReplacer = function(separatorIn) {
    var storedBefore = false;
    var separator = separatorIn;
    this.replace = function(oldValue, newValue) {
...
PostAffCookieManager = function() {
    var flash = null, flashVersion = null;
    ...
    this.saveVisitorToHttpCookie = function(visitorId) {
        PostAffCookie.setHttpCookie(visitorCookie.name, visitorId);
    };
    ...
    this.readFlashCookies = function(cookies) {
        var params = new PostAffParams('pap.swf');
        params.addParam('a', 'r');
...
PostAffParams = function(scriptName) {
    ...
    this.toString = function() {
        return this.script + this.encodeParams();
    };
};
PostAffParams.parse = function(url) {
    ...
    if (parts.length > 1) {
        ...
        for (var i = 0; i < parameters.length; i++) {
            ...
            params.addParam(parseParam[0], parseParam[1]);
        }
    }
    return params;
}
...
PostAffCookie = function(name) {
    ...
    this.deleteCookie = function() {
        PostAffCookie.deleteHttpCookie(this.name);
    };
};
...
PostAffCookie.setHttpCookie = function(name, value, expired) {
    if (expired == null) {
        var theDate = new Date();
        var expired = new Date(theDate.getTime() + 31536000000 * 10);
    }
    document.cookie = name + '=' + encodeURIComponent(value) + ';expires=' + expired.toGMTString() + ';path=/';
};
PostAffCookie.deleteHttpCookie = function(name) {
    expired = new Date(0);
    PostAffCookie.setHttpCookie(name, '', expired);
};

PostAffRequest = function(cookieManager) {
...
PostAffRequest.prototype.callTrackScript = function() {
    ...
    scriptElement = document.getElementById(PostAffTracker.getIntegrationElementId());
    scriptElement.parentNode.insertBefore(trackingScriptElement, scriptElement.nextSibling);
};

PostAffInfo = function(cookieManager) {
    this.cookieManager = cookieManager;
    var affiliateId, campaignId, accountId;
    var received = false;
    ...
    this.setAffiliateInfo = function(affiliateIdIn, campaignIdIn) {
...
PostAffTrackingRequest = function(cookieManager, actions, accountId, skipIframeCheck) {
    this.cookieManager = cookieManager;
...
PostAffTrackingRequest.prototype.getTrackingParams = function() {
    return new PostAffParams("track.php");
}
PostAffTrackingRequest.prototype.fillTrackingParams = function() {
    ...
    params.addParam('url', PostAffParams.replaceHttpInText(window.location.protocol + "//" + window.location.host + "/" + window.location.pathname));
    params.addParam('referrer', PostAffParams.replaceHttpInText(PostAffParams.getReferrer()));
    params.addParam('getParams', PostAffTrackingRequest.getGetParams().toString());
...
if (PostAffTracker == undefined) {
    var PostAffTracker = new function() {
        ...
        this.writeVisitorIdToAttribute = function(id, attributeName, urlParamName, separatorIn, valueAsUrl) {
            this._cmanager.loadHttpCookies();
            var writer = new PostAffAttributeWriter(id, attributeName, urlParamName, this._getSeparator(separatorIn), valueAsUrl);
            writer.writeAttribute(this._getAccountId()
            + this._cmanager.getVisitorIdOrSaleCookieValue());
            ...
            var self = this;
            setTimeout(function() {
                writer.writeAttribute(self._getAccountId()
        ...
        this.writeCookieToCustomField = function(id, separatorIn, urlParamName) {
            if (urlParamName == undefined) {
                urlParamName = null;
            }
            this.writeVisitorIdToAttribute(id, 'value', urlParamName, separatorIn);
        };
        this.writeCookieToLink = function(id, urlParamName, separatorIn) {
            this.writeVisitorIdToAttribute(id, 'href', urlParamName, separatorIn);
        };
        ...
        this.onTrackingFinished = function() {
            ...
            for (var i = 0; i < this.executeOnResponceFinished.length; i++) {
                this.executeOnResponceFinished[i]();
            }
        }
        ...
        this._setAffiliateInfo = function(affiliateId, campaignId) {
            affInfo.setAffiliateInfo(affiliateId, campaignId);
...
function setAffiliateInfo(userId, campaignId) {
    PostAffTracker._setAffiliateInfo(userId, campaignId);
}
function papTrack() {
