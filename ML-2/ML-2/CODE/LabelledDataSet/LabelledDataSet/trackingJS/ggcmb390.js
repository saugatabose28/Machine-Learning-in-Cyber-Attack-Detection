...
function gg() {
    ...
    this.ggJsLoaded = function() {
        ...
        for (var i = 0; i < cN.length; i += 6)
            ggJsMet.C(cN[i], cN[i + 1], cN[i + 2], cN[i + 3], cN[i + 4], cN[i + 5]);
        cN = [];
    }
};
function Metrics() {
    ...
    function getut(A) {
        ...
        if (au == 1)
            ...
            if (A == 0)
                cX += "&rnd=" + Math.floor(Math.random() * 100000);
            cX += "&tp=gg";
        }
        return cX;
    };
    function dp(bD, curval, ac) {
        var x2 = M("iag_" + bD, ac);
        if (x2 != null && x2 != "")
            return "&pr=iag." + bD + "," + encodeURIComponent(x2);
        if (curval == null)
            return "";
        return curval;
    };
    function dn(ac, p1, p2, p3, p4) {
        ...
        if (ac == 0 || p2 != "preroll") {
            ...
            if (x2 == null || x2 == "")
                x2 = M("category", 0);
            if (x2 != null && x2 != "")
                pgm = "&pr=iag.pgm," + encodeURIComponent(x2);
            else 
                pgm = "&pr=iag.pgm,general";
            x2 = M("title", 0);
            if (x2 != null && x2 != "")
                epi = "&pr=iag.epi," + encodeURIComponent(x2);
            else 
                epi = "&pr=iag.epi," + encodeURIComponent(cU["Title"][0].substr(0, 255));
            seg = "&pr=iag.seg,";
            if (p4 > 1)
                seg += encodeURIComponent(p4);
            else 
                seg += "1";
            x2 = M("pd", 0);
            if (x2 == null)
                x2 = cV.pd;
            if (x2 != null && x2 != "")
                pd = "&pr=iag.pd," + encodeURIComponent(x2);
            else 
                pd = "";
            x2 = M("oad", 0);
            if (x2 != null && x2 != "")
                oad = "&pr=iag.oad," + encodeURIComponent(x2);
            else 
                oad = "";
            sid = dp("sid", sid, 0);
            tfid = dp("tfid", tfid, 0);
        ...
        if (ac) {
            ...
            iaguc = sid + tfid + bcr + pgm + epi + seg + pd + oad + ifp + icust1;
    };
    ...
    function report(A) {
        if (A == 0 && cV.legacybeacon == "true") {
            var legacy_urlTail = "&tl=" + (cU["Title"][o] ? cU["Title"][o].substr(0, 128) : '');
            var legacy_x2 = M("censuscategory", o);
            if (legacy_x2 != null && legacy_x2 != "")
                legacy_urlTail += "&cg=" + encodeURIComponent(legacy_x2);
            if (o)
                legacy_urlTail += "&c3=st,a";
            legacy_urlTail += "&rnd=" + Math.floor(Math.random() * 100000);
            ca.legacyReport("legacyBeacon||;||" + cV.clientid, legacy_urlTail);
        }
        var cX = getut(A);
        ca.report(aL(), A, cX);
        _currentMsgSize = 0;
    ...
    this.init = function(I, userId, ggAddress, fpar) {
        ...
        if (au != 1) {
            if (cV.sfcode == undefined || cV.sfcode.length != 2)
                ggAddress = AddressProtocol + "//secure-us.imrworldwide.com/cgi-bin/m?";
            else 
                ggAddress = AddressProtocol + "//secure-" + cV.sfcode + ".imrworldwide.com/cgi-bin/m?";
            ...
            if (cV.aJ != undefined && cV.aJ != "")
                ggAddress += "&c4=mn," + encodeURIComponent(cV.aJ);
            ggAddress += "&cc=1";
        }
        _address = ggAddress;
    ...
    this.aD = function(i, ts) {
        ...
        if (cU["CS"][i] != 0 && cU["PA"][i] > 0 && cU["VL"][i] > 0) {
            ...
            if (cU["SR"][i] > 0);
            m += 10;
            if (cU["SH"][i] > 0 || cU["SV"][i] > 0 || cU["PB"][i] > 0 || cU["CI"][i] > 0)
                m = 99;
            if (cU["II"][i] > 0)
                m += 10;
...
function cz(version, userId, address, aI, an, metricsObj) {
    ...
    this.legacyReport = function(F, cX) {
        cg.report(F, cX);
    };
    this.bp = function(cX) {
        var F = ce.bx(viewst);
    ...
    this.af = function(bD) {
        if (O) {
            ...
            if (l_lfunctionType == 5 || l_lfunctionType == 8)
                bD = '';
            viewst = bD;
        }
    }
...
function cw(cD, bI, bk, an) {
    ...
    function bB(cY, cX) {
        ...
        if (cb == 'GET')
            _i = new Image(1, 1);
        var _getLoggerAddress = aV;
        var fireLegacyBeacon = false;
        if (cY.indexOf("legacyBeacon||;||") > - 1) {
            var clientID = cY.split("||;||")[1];
            var strtPos = aV.indexOf("ci=") + 3;
            var endPos = aV.indexOf("&", strtPos);
            var replaceString = aV.substring(strtPos, endPos);
            _getLoggerAddress = aV.replace(replaceString, clientID);
            fireLegacyBeacon = true;
        }
        if (fireLegacyBeacon)
            _i.src = _getLoggerAddress + cX;
        else {
            if (au == 1)
                _i.src = _getLoggerAddress + cX + '?HEX40=' + cY;
            else 
                _i.src = _getLoggerAddress + cX + "HEX40%3D" + cY;
        }
    }
};
function bo() {
...
function EventProcessor() {
    ...
    this.genericEvent = function(eventType, date, d, f, j, k) {
        if (genEventType == eventType && dc == d && dh == f && dg == j && de == k)
            return null;
    ...
    this.cs = function(eventType, date, d, f, j, k) {
        if (ggEventType == eventType && cq == d && cn == f && co == j && cj == k)
            return null;
        var cW = null;
        if (eventType == 1) {
            this.bW = eventType;
