...
function topError(text, opts) {
    ...
    if (text.message) {
        ...
        opts.stack = e.stack;
        if (e.stack && __debugMode) 
            text += '<br/>' + e.stack.replace(/\n/g, '<br/>');
        try {
            console.log(e.stack);
        } catch (e2) {};
    }
    ...
    if (!__dev && !ge('debuglogwrap')) {
        ...
        ajax.plainpost('/errors.php', extend(opts, {
            msg: opts.msg || text,
            module: (window.cur || {}).module, id : vk.id, host : locHost, lang : vk.lang, loc : (window.nav || {}).strLoc, realloc : location.toString()
        }));
    }
}
...
function clone(obj, req) {
    ...
    for (var i in obj) {
        if (/webkit/i.test(_ua) && (i == 'layerX' || i == 'layerY')) 
            continue;
    ...
    update: function() {
        for (var p in this.now) {
            if (isArray(this.now[p])) 
                ...
                this.el[p] != undefined ? (this.el[p] = this.now[p]) : setStyle(this.el, p, this.now[p]);
        }
    },
...
function scrollToY(y, speed, anim, topLink) {
    ...
    if (browser.msie6) {
        if (data(pageNode, 'tween')) 
            data(pageNode, 'tween').stop(false);
    } else {
        if (data(bodyNode, 'tween')) 
            data(bodyNode, 'tween').stop(false);
        if (data(htmlNode, 'tween')) 
            data(htmlNode, 'tween').stop(false);
    }
...
function notaBene(el, color, nofocus) {
    ...
    if (!nofocus) 
        elfocus(el);
    if (data(el, 'backstyle') === undefined) 
        data(el, 'backstyle', el.style.backgroundColor || '');
...
if (__debugMode) {
    function __checkData() {
        ...
        for (var i in vkCache) {
            ...
            var id = el.id;
            if (!id) 
                el.id = id = '__vk' + irand(1000000, 9999999);
            if (ge(id) == el) 
                continue;
            var d = [];
            for (var j in c) {
                if (j == '__elem' || j == 'handle' && c.events) 
                    continue;
                if (j == 'events') {
                    var e = [];
                    for (var k in c[j]) 
                        e.push(k + '(' + c[j][k].length + ')');
                    d.push('{' + e.join(', ') + '}');
                } else {
                    d.push(j);
                }
...
var layers = {
    ...
    push: function(onShow) {
        ...
        if (cur.pvShown && cur.pvListId != 'temp') {
            ...
            clayer = ['video', mvcur.videoRaw, mvcur.listId, {
...
function updSideTopLink(resized) {
    ...
    if (!vis) {
        ...
        if (f !== false) 
            toggleClass(_stlLeft, 'over_fast', hasClass(_stlLeft, 'over') && f);
        setStyle(_stlLeft, {
...
function toggleGlobalPlayer(open) {
    ...
    if (!_gp) 
        return;
    if (open) {
        if (cur.gpHidden || cur.gpShowing || _a && _a.gpHidden || ge('head_play_btn') && scrollGetY() < 33) 
            return;
        // updGlobalPlayer();
        cur.gpShowing = true;
        cur.gpHiding = false;
...
function onBodyResize(force) {
    ...
    if (w.lastWindowHeight != dheight || force === true) {
        ...
        if (_pads.layerBG) 
            _pads.layerBG.style.height = dheight + 'px';
    ...
    if (changed && w.curRBox && w.curRBox.boxes && window.getWndInner) {
        ...
        each (curRBox.boxes, function() {
            this._wnd_resize(wndInner[0], wndInner[1])
        });
    }
...
function onDocumentClick(e) {
    ...
    if (_pads.shown) {
        if (e && e.target && e.target.tagName && e.target.tagName.toLowerCase() != 'input' && cur.__mdEvent && e.target != cur.__mdEvent.target) 
            return;
        for (var el = e.target; el != bodyNode; el = domPN(el)) {
            if (!el || el.id && el.id.match(/^(box_|mv_|wk_|pad_)?(layer|pad)_(bg|wrap)$/) || (el.className || '').match(/(^|\s)tt(\s|$)/)) 
                break;
        }
        if (el == bodyNode) 
            Pads.hide();
    ...
    if (cur.onMouseClick && cur.onMouseClick(e)) 
        return;
    ...
    path = path[1];
    if (path.indexOf('.php') > 0 || /*path.indexOf('/') > 0 || */
    path.match(/^(doc\-?\d+_\d+|graffiti\d+|reg\d+|images|utils|\.js|js\/|\.css|css\/|source\b)/)) {
        return true;
    }
...
function domStarted() {
    ...
    if (!browser.mobile) {
        ...
        addEvent(window, 'focus', function(e) {
            _wf = 1;
            if (__needBlur /* || browser.mac*/
            ) {
                // also need __afterFocus = true on mac, e.g. in IM.onWindowFocus
                return; // opera fix
            }
...
function initFixedMenu() {
    var lnks = {
        fr: '/friends?section=requests',
        ph: '/albums?act=added',
        vid: '/video?section=tagged',
        msg: '/im',
        gr: '/groups',
        nws: '/feed?section=notifications',
        ap: '/apps'
    }, text = [], i, el;
    for (i in lnks) {
    ...
    re(_fMenu);
    extend(window, {
        _fMenu: _stlSide.appendChild(ce('div', {
            id: 'fmenu',
            innerHTML: text.join(''),
...
function ajx2q(qa) {
    ...
    for (var key in qa) {
        if (qa[key] == null || isFunction(qa[key])) 
            continue;
        if (isArray(qa[key])) {
...
var stManager = {
    ...
    add: function(files, callback, async) {
        ...
        for (var i in files) {
            ...
            if ((opSpeed || __debugMode) && !(browser.iphone || browser.ipad) && f != 'common.js' && f != 'common.css' && stVersions[f] > 0 && stVersions[f] < 1000000000) 
                stVersions[f] += irand(1000000000, 2000000000);
...
var ajax = {
    ...
    _post: function(url, q, o) {
        if (!q.captcha_sid && o.showProgress) 
            o.showProgress();
        var cacheKey = false;
        ...
        if (o.local) 
            processResponse = vkLocal(processResponse);
        var done = function(text, data) {
            // data - for iframe transport post
            if (o.bench) {
...
function leftAdBlockClose(blockIdSuffix, closeLink) {
    ...
    setStyle('left_hide' + blockIdSuffix, {
        visibility: 'hidden'
    });
    ...
    function onClose() {
        animate('ads_ad_close_info_' + blockIdSuffix, {
            opacity: 1
        }, 200, onCloseComplete);
    }
    function onCloseComplete() {
        setStyle('ads_ad_box2_' + blockIdSuffix, {
            visibility: 'hidden'
        });
    }
}
...
function leftBlockToggleFriend(mid, type, hash, act, ev) {
    ...
    ajax.post('al_friends.php', {
        ...
        showProgress: function() {
            ...
            ge('left_friend_status_' + mid).innerHTML = '<img src="/images/upload' + (window.devicePixelRatio >= 2 ? '_2x' : '') + '.gif" />';
        },
        hideProgress: function() {
            hide('left_friend_status_' + mid);
        },
...
function leftBlockUnpaidGiftsHide(blockId, hash) {
    return showFastBox({
        ...
        leftBlockHide(blockId, hash);
        re('payments_box_unpaid_gifts_info');
        curBox().hide();
    }, getLang('global_cancel'));
}
...
function handlePageView(params) {
    ...
    if (width != vk.width || widthDec != vk.width_dec) {
        ...
        if (ge('ts_wrap') && hasClass(ge('ts_wrap'), 'vk')) 
            ge('ts_wrap').style.width = (width - 191) + 'px';
...
function handlePageParams(params) {
    ...
    if (params.pvbig !== undefined) 
        vk.pvbig = params.pvbig;
    if (params.pvdark !== undefined) 
        vk.pvdark = params.pvdark;
    ...
    var lnks = ['friends', 'albums' + vk.id, 'video', '', 'notes', 'groups', 'events', 'gifts.php?act=wishlist', 'apps', (intval(cnts[9]) > 0) ? 'exchange' : 'ads?act=office&last=1', 'feed' + (ge('l_nwsf') ? '?section=notifications' : ''), 'pages', 'docs'];
    var adds = ['', 'act=added', 'section=tagged', '', 'act=comments', '', 'tab=invitations', '', '', (intval(cnts[9]) > 0) ? 'act=overview&status=-1' : ' ', ge('l_nwsf') ? '' : 'section=notifications'];
...
var nav = {
    ...
    go: function(loc, ev, opts) {
        ...
        if (_a && aid && (_a.gpDisabled || !ge('gp')) && !opts.back) 
            _a.stop();
            ...
            for (var i = 0, l = globalHistory.length; i < l; ++i) {
                if (globalHistory[i].loc == strLoc) {
                    ...
                    setTimeout(function() {
                        ...
                        for (var i = 0, l = cur._back.show.length; i < l; ++i) 
                            cur._back.show[i]();
                        if (onback) {
                            for (var i = 0, l = onback.length; i < l; ++i) 
                                onback[i]();
                        }
                        nav.setLoc(strLoc);
                        var b = h.back || {};
        ...
        var done = function(title, html, js, params) {
            if (params.zen) {
                window.location.href = params.loc;
                return;
            }
            ...
            if (_back) {
                ...
                for (var i = 0, l = cur._back.hide.length; i < l; ++i) 
                    cur._back.hide[i]();
            ...
            if (!opts.noscroll && !params.noscroll) 
                scrollToTop(0);
            ...
            lTimeout(function() {
                ...
                if (_a && aid && _a.showCurrentTrack) 
                    _a.showCurrentTrack();
                if (!isVisible('ts_wrap') && hasClass(ge('ts_wrap'), 'vk')) 
                    TopSearch.toggleSettings();
...
// Time offset stuff
//
if (vk.time && !browser.opera_mobile) 
    setTimeout(function() {
...
function placeholderSetup(id, opts) {
    ...
    if (o.reload) {
        ...
        if (prel && hasClass(prel, 'input_back_wrap')) 
            re(prel);
    }
...
function MessageBox(options, dark) {
    ...
    if (!options.progress) 
        options.progress = 'box_progress' + guid;
    ...
    var retBox = _message_boxes[guid] = {
        ...
        setButtons: function(yes, onYes, no, onNo) {
            ...
            if (!yes) 
                return b.addButton(box_close);
            if (no) 
                b.addButton(no, onNo, 'no');
...
function showCaptchaBox(sid, dif, box, o) {
    var done = function(e) {
        if (e && e.keyCode !== undefined && e.keyCode != 10 && e.keyCode != 13) 
            return;
...
function lockButton(el) {
    ...
    if (!btn && !hasClass(el, 'file_button')) {
        ...
        if (!elButton || elButton.parentNode != el) 
            return;
        el = elButton;
        btn = true;
    }
...
function disableButton(el, disable) {
    if (!(el = ge(el)) || el.tagName.toLowerCase() !== 'button') 
        return;
    if (hasClass(el, 'flat_button')) {
...
function disableFlatButton(el, disable) {
    ...
    if (disable) {
        ...
        if (disabledEl && hasClass(disabledEl, 'button_disabled')) 
            re(disabledEl);
        show(el);
    }
}
...
function checkTextLength(maxLen, inp, warn, nobr, cut, force, utf) {
    ...
    if (inp.lastLen === value.length && !force) 
        return;
...
function goAway(lnk, prms, e) {
    ...
    return !showBox('away.php', params, {
        dark: true
    }, e);
}
...
function renderFlash(cont, opts, params, vars) {
    ...
    if (__debugMode && stVersions[f] < 1000000) 
        stVersions[f] += irand(1000000, 2000000);
...
function playAudioNew() {
    ...
    if (args[args.length - 1] !== false) 
        args = Array.prototype.slice.apply(arguments).concat([true]);
...
function showTooltip(el, opts) {
    ...
    stManager.add(['tooltips.js', 'tooltips.css'], function() {
        ...
        if (!el.showing || cur.cancelTooltip) 
            return;
        _cleanHide(el);
...
function updateMoney(balance) {
    if (balance === undefined || balance === false) 
        return;
    vk.balance = balance;
...
function zNav(changed, opts, fin) {
    ...
    if (zt) {
        ...
        case 'photo_tag':
            ...
            break;
        case 'video_tag':
            showVideoTags(zt[2], extend(opts, {
                onFail: onFail,
                noHistory: true
            }));
            return false;
            break;
        case 'video':
            showVideo(zt[2], zt[3], extend(opts, {
                onFail: onFail,
                noLocChange: 1
            }));
            return false;
            break;
        case 'screen':
            stManager.add(['index.js'], function() {
                setTimeout(JoinPhotoview.show.pbind(intval(zt[2])), 0);
            });
            return false;
            break;
        case 'single':
...
function showPhoto(photoId, listId, options, ev) {
    ...
    if (checkEvent(ev) || (cur._editMode && cur._editMode(ev))) 
        return;
    var stat = ['photoview.js', 'photoview.css', 'page.js', 'page.css'], phv = window.Photoview;
    if (options.img) {
        options.showProgress = showGlobalPrg.pbind(options.img, {
...
function showInlineVideo(videoId, listId, options, ev, thumb) {
    ...
    options.onDone = function (title, html, js, opts) {
        ...
        videoBg && setStyle(geByTag1('img', videoBg), {
            width: w,
            height: h
        });
        try {
...
function animateCount(el, newCount, opts) {
    ...
    setStyle(wrapEl, {
        width: (constEl1 && constEl1.offsetWidth || 0) + (constEl2 && constEl2.offsetWidth || 0) + bigW + 0
    })
...
var Chat = {
    ...
    init: function() {
        Chat.wrap = ce('div', {
            ...
            innerHTML: '<div class="chats_sp chat_cont_sh_top"></div><div class="chat_cont_scrolling"><div class="chat_onl_height"></div></div><div class="chats_sp chat_cont_sh_bottom"></div><a class="chat_tab_wrap" id="chat_tab_wrap" onclick="Chat.showFriends()" onmouseover="Chat.showTT();"><div class="chat_onl_cont"><div class="chat_onl" id="chat_onl"></div><div class="chats_sp chat_onl_icon"></div></div></a>'
        });
...
TopSearch = {
    ...
    init: function() {
        ...
        addEvent(tsInput, 'focus', function() {
            TopSearch.deselect();
            if (trim(val(this))) 
                addClass(tsCont.firstChild, 'active');
        ...
        addEvent(document, 'mousedown', function(e) {
            // removed touchstart because of feed lags @izhukov
            if (!isVisible('ts_wrap') && hasClass(ge('ts_wrap'), 'vk')) 
                TopSearch.toggleSettings();
            tsInput.blur();
            TopSearch.toggleInput(false);
    ...
    highlightInput: function(s) {
        if (true || !hasClass(ge('ts_wrap'), 'vk')) 
            return false;
    ...
    toggleInput: function(s, noAnimation) {
        ...
        if (s) {
            ...
            if (vk.rtl) {
                var newBG = browser.msie8 || browser.msie7 ? {
                    backgroundPositionX: '152px'
                }
                : {
                    backgroundPosition: '152px 5px'
                };
                animate(tsInput, newBG, {
                    duration: duration,
                    transition: Fx.Transitions.easeOutCubic
                });
            }
            ...
            if (!trim(val(tsInput))) {
                ...
                if (vk.rtl) {
                    var newBG = browser.msie8 || browser.msie7 ? {
                        backgroundPositionX: '112px'
                    }
                    : {
                        backgroundPosition: '112px 5px'
                    };
                    animate(tsInput, newBG, {
                        duration: duration,
                        transition: Fx.Transitions.easeOutCubic
    ...
    getList: function(type) {
        switch (type) {
        case 'friends':
            return (this.lists['friends'] || /*window.curFastChat && curFastChat.friends||*/
            this.topFriends || {});
        case 'publics':
        case 'events':
        case 'groups':
    ...
    getSimilarQueries: function(q) {
        ...
        if (t = parseLatin(q)) 
            queries.push(t);
        if (t = TopSearch.parseLatKeys(q)) 
            queries.push(t);
        if (t = TopSearch.parseCyr(q)) 
            queries.push(t);
        return queries;
    },
    searchCache: function(listName, q) {
        ...
        if (this.cache[listName][q] !== undefined) 
            return queries;
    ...
    row: function(mid, href, photo, name, online, re, hintType, info) {
        var peer = 0, typeAttr = '';
        if (re) 
            name = name.replace(re, '$1<em class="ts_clist_hl">$2</em>');
    ...
    writeBox: function(peer) {
        if (window.curFastChat && curFastChat.inited && window.FastChat) {
            // black movable rbox
            FastChat.selectPeer(peer);
        } else if (peer > 0 && peer < 2e9) {
            showWriteMessageBox(false, peer);
        } else {
            // no box for multichats or emails
            nav.go('/im?sel=' + peer);
        }
    }
}
...
var Pads = {
    preload: function(id) {
        ...
        if (id == 'msg') 
            st.push('pads_im.css', 'pads_im.js', 'page.css', 'page.js');
        stManager.add(st, function() {
            Pads.preload(id);
        });
    },
    show: function(id, ev) {
        if (checkEvent(ev) === true || browser.msie6) 
            return;
        var st = ['pads.css', 'pads.js', 'notifier.js'];
        if (id == 'msg') 
            st.push('pads_im.css', 'pads_im.js', 'page.css', 'page.js');
...
function imagesLoader(cont, options) {
    ...
    obj.processLoad = function() {
        ...
        if (loading_cnt >= opts.load_limit) 
            return;
...
function IframeLoader() {
    ...
    function iframeDoc(i) {
        try {
            ...
            if (i.contentWindow && i.contentWindow.document) 
    ...
    function getImgHtml(i) {
        if (doc && doc.body) 
            return '<img id="___img' + i + '" />';
        else 
            return '<img class="___img' + i + '" />';
    }
    function getImg(i) {
        if (doc && doc.body) 
            return doc.getElementById('___img' + i);
        else 
            return geByClass1('___img' + i, body);
    }
    function init() {
