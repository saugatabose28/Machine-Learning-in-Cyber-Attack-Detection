...
(function($) {
    ...
    $.fn.extend({
        hideSuperfishUl : function() {
            ...
            o.retainPath = false;
            var $ul = $(['li.', o.hoverClass].join(''), this).add(this).not(not).removeClass(o.hoverClass); //Modified to allow support for slide effect.
            //Fix for simplemenu.
            $ul.find('>ul.menu').hide().css('visibility', 'hidden');
            o.onHide.call($ul);
            return this;
        },
...
(function ($) {
    $(document).ready(function() {
        ...
        //Fade in active 
        $('ul.nice-menu li.active-trail ul').fadeIn();
    });
})(jQuery);;
...
(function ($) {
    ...
    $(document).ready(function() {
        ...
        $('div#block-views-participating_networks-block_1 ul li:gt(4), div.view-participating-networks ul li:gt(4)').css('display', 'none');

        $('div#block-views-participating_networks-block_1 ul li:gt(4), div.view-participating-networks ul li:gt(4)').find('img').each(function() {
            images.push({
                'src': $(this).attr('src'),
...
window.third_party_cookie_test_step1_loaded = function() {
    // At this point, a third-party domain has now attempted to set a cookie
    var step2_url = 'http://www.naicookietest.com/third_party_test.js.php?step=two';
    var results_element = document.getElementById('content-area');
    var step2_element = document.createElement('script');

    // Load the second part of the test (reading the cookie)
    step2_element.setAttribute('src', step2_url);
    results_element.appendChild(step2_element);
}

window.third_party_cookie_test_step2_loaded = function(cookie_success) {
    var results_element = document.getElementById('third_party_cookie_test_results');

    if (cookie_success == true) {
        $.cookie("client_tpc", "enabled", {
            path: "/" 
        });
    } else {
        $.cookie("client_tpc", "disabled", {
            path: "/" 
        });
        // Show third party cookies text
        $(".tpc").css('display', 'block');
    }
};
(function() {
