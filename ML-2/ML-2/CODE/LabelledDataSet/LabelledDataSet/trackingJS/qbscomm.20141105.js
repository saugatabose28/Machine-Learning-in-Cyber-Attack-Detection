        ...
        GDTDefine('gdt:mod/util.js', function(require, exports, module) {
            ...
            UMod.string = {
                ...
                escString: function(str) {
                    var t = UMod.string.RegExps.escString, h = UMod.string.RegExps.escHTML;
                ...
                cut: function(str, bitLen, tails) {
                    ...
                    var len = str.length, i = Math.min(Math.floor(bitLen / 2), len), cnt = UMod.string.getRealLen(str.slice(0, i));
            ...
            UMod.dom = {
                ...
                getPosition: function(el) {
                    var xy = UMod.dom.getXY(el), size = UMod.dom.getSize(el);
                    return {
                        "top": xy[1],
                        "left": xy[0],
                        "width": size[0],
                ...
                getXY: function(el, doc) {
                    ...
                    if (el) {
                        if (_doc.documentElement.getBoundingClientRect && el.getBoundingClientRect) {
                            var box = el.getBoundingClientRect(), oDoc = el.ownerDocument, _fix = UMod.userAgent.ie ? 2: 0;
            ...
            UMod.JSONGetter = (function() {
                ...
                function load(url, callback, ecb, opts) {
                    var script = document.createElement('script'), head, doc = document, global = window, _ecb, charset, clear, timer, _gcname, done = false;
                    ...
                    script.onload = script.onreadystatechange = function(evt) {
                        ...
                        var rs = script.readyState, ready;
                ...
                function jsonp(url, params, callback, ecb, opts) {
                    var cbnameKey, query = (url || '').indexOf('?')===-1 ? '?': '&', key;
                    opts = opts || {};
            ...
            UMod.event = {
                ...
                trigger: function(obj, eventType) {
                    var l = obj && UMod.event._eventListDictionary[obj.eventsListUID], handlers = l && l[eventType] && l[eventType].handlers, i;
                    if (handlers) {
                        try {
                            for (i in handlers) {
                ...
                removeEvent: function(obj, eventType, fn) {
                    var cfn = fn, res = false, l = UMod.event._eventListDictionary, r;
                    if (!obj ||!fn) {
                        return res;
                    }
