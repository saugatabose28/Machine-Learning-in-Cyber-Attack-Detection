...
if (window.addEventListener)
    window.addEventListener("message", adsGUIDFn, false);
...
function adsDisableMSP() {
    adsMSP = 0
}
function adsDisableMobDyn() {
    adsMobDyn = 0
}
...
function adsLoadedSync() {
    ...
    for (var i = 0; i < z; i++) {
        ...
        if (v.textAd != 1 && v.iframe != 1 && v.delayed)
            v.LoadAd();
    }
    adsLoadSync = 1;
}
function adsLoadUAC() {
    ...
    if (adsDebug > 0) {
        ...
        if (x[1] && x[1] == 'c')
            adsUACH = 'http://cdn.atwola.com/_media/uac/'
        else if (x[1] && x[1] == 's')
            adsUACH = 'https://s.aolcdn.com/ads/'
        else 
            adsUACH = 'http://browsertest.web.aol.com/ads/';
        u = x[0];
    }
...
if ((adsUAC > 0 || adsDebug > 0)&&!window.adsUACH)
    adsLoadUAC()
else {
    if (window.adsIn != 1) {
        adsIn = 1
        ...
        if ((adsUA.indexOf('mobile')>-1) || (/android|iphone|ipad|playbook|hp-tablet|kindle|silk|webos|blackberry|opera mini/i).test(navigator.appVersion))
            adsDev = '1';
        if (!adsDev)
            adsDev = '2';
        ...
        function adUACInit() {
            if (!adsSecure) {
                ...
                if (w.addEventListener)
                    w.addEventListener("load", adsTacFn, false)
                else if (w.attachEvent)
                    w.attachEvent("onload", adsTacFn)
            }
            if (/(^|;)?RSP_COOKIE=.*?&name=(.*?)(&|;|$)/i.test(document.cookie))
                adsESN = '&ESN=' + unescape(RegExp.$2);
            var at = adsLo.search(/atwcrpr=/i);
            adsIE = (navigator.appName == "Microsoft Internet Explorer");
            if (adsLo.search(/atwdistcode/i) > 0)
                adsDisableTacoda()
            if (at > 0) {
                adsCA = adsLo.substr(at + 8).split(/\||;/);
                adsCp = 1;
                var z = adsCA.length;
                for (var i = 0, k = 0; i < z; i = i + 4, k++) {
                    adsCF[k] = adsCA[i];
                    adsCW[k] = adsCA[i + 1];
                    adsCH[k] = adsCA[i + 2];
                    adsCAd[k] = adsCA[i + 3]
        ...
        function adSetHtNmAT(v) {
            ...
            if (adsHt.charAt(adsHt.length - 1) == '/')
                adsHt = adsHt.substring(0, adsHt.length - 1);
        }
        function adSetAMS() {}
        ...
        function adsReloadAd(d, m, v) {
            ...
            if (s) {
                s = unescape(s);
                if (m)
                    s = s.replace(/alias=(.*?);/, "alias=" + m + ";").replace(/kvmn=(.*?);/, "kvmn=" + m + ";");
                var i = s.indexOf(';grp='), u = ''
                if (i==-1)
                    u = s.replace(/ /, "") + " "
                else 
                    u = s.substring(0, i + 5) + dt
        ...
        function adsDevilObj(d, w, h, f, doc, r) {
            ...
            try {
                ...
                if (!adsRRDevil) {
                    if (f.sz == '300x250,300x600,300x1050' && h != '1050')
                        adsRRDevil = 'n';
            ...
            try {
                if (window.adsDevilAd.hasOwnProperty('resized'))
                    adsDevilAd.resized(d, w, h);
                if (window.adsDevilAd.hasOwnProperty('adinfo'))
                    adsDevilAd.adinfo(n, d, w, h);
                if (window.adsDevilAd.hasOwnProperty('adinfo2'))
                    adsDevilAd.adinfo2(n, d, w, h);
                if (window.adsDevilAd.hasOwnProperty('moat'))
                    adsDevilAd.moat(n, d, w, h, tx);
                if (w == '300') {
        ...
        function adsRMIFOnL(w, d) {
            ...
            if (adsQuigo == 0 && (/aolSize=["']([\d]*?)\|([\d]*)["']/i.test(aD1)) && (unescape(RegExp.$2) > 1)) {
                ...
                if (/ACE_AR(.*?)Size(.*?)[,}]/i.test(aD1)&&!a) {
                    ...
                    if (!adsMob) {
                        if (/img (.*?)width=["']?(.*?)(\"|\'| )/i.test(aD1))
                            wi = parseInt(RegExp.$2);
                        if (/img (.*?)height=["']?(.*?)[\"|\'| ]/i.test(aD1))
                            h = parseInt(RegExp.$2);
                    }
                    if (!wi ||!h || wi < 2 || h < 2) {
                        if ((v.childNodes.length == 1) || (d.adsWidth && d.adsHeight)) {
                            if (d.adsWidth && d.adsHeight) {
                                wi = d.adsWidth;
                                h = d.adsHeight;
                            } else {
                                if (s) {
                                    wi = s.offsetWidth
                                    if (adsIE && parseInt(adsUA.charAt(adsUA.indexOf('trident/') + 8)) < 5)
                                        h = s.offsetHeight
                                    else 
                                        h = aD.offsetHeight
                                }
        ...
        function adsLoadAd() {
            ...
            if (this.iframe) {
                ...
                if ((document.domain != location.hostname) && (this.adPage.indexOf('#')==-1))
                    this.adPage = this.adPage + '#' + document.domain
                if (this.adPage) {
                    if (f.inV == '0') {
                        f.src = this.adPage;
                    } else {
        ...
        function adsGetValues() {
            ...
            if (v) {
                ...
                if (!x)
                    r = 'kvdl=' + r.substring(0, r.length - 1) + ';';
                else 
                    r = '';
            }
            ...
            if (adsATOth.indexOf('kvugc')==-1) {
                s = 'kvugc=';
                if (window.adSetUGC == 0)
                    s += '0;'
                else if (window.adSetUGC == 1)
                    s += '1;'
                else {
                    if (adsATOth.indexOf('cmsid')==-1)
                        s += '0;'
                    else 
                        s += '1;'
                }
            ...
            function f(c, a) {
                ...
                return function() {
                    ...
                    b || (b = d.setTimeout(g, a))
                }
            }
            function i(c, a, b) {
                if (!j)
                    j = k.body;
                var g = c, e = 0, f = j.offsetHeight, h = d.innerHeight || k.documentElement.clientHeight || j.clientHeight || 0, i = k.documentElement.scrollTop || d.pageYOffset || j.scrollTop || 0, m = c.offsetHeight || 0;
                if (!c.sonarElemTop || c.sonarBodyHeight !== f) {
                    if (g.offsetParent) {
                        do 
                            e += g.offsetTop;
                ...
                a = a === l ? 0 : a;
                return !(c.sonarElemTop + (b ?
                0 : m * b) < i - a)&&!(c.sonarElemTop + (b ? m * b : 0) > i + h + a)
            }
            function e() {
                var c, a, b;
                for (c in h)
                    if (h.hasOwnProperty(c) && (a = h[c], a.scrollin || a.scrollout))
                        if (b = i(a.elem, a.distance, a.visibility), b !== a.detected)
                            b ? a.scrollin && a.scrollin.call(a, a.elem) : a.scrollout && a.scrollout.call(a, a.elem), a.detected = b
            }
            var j, h = [];
            d.addEventListener ? (d.addEventListener("scroll", f(e, 200), !1), d.addEventListener("resize", f(e, 200), !1)) : d.attachEvent && (d.attachEvent("onscroll", f(e, 200)), d.attachEvent("onresize", f(e, 200)));
            d.adsSonar = function(c,
            a, b) {
                if ("object" === typeof a)
                    b = a;
                ...
                if (b.scrollin || b.scrollout)
                    b.elem = c, h.push(b), e();
                return i(c, b.distance, b.visibility)
            }
        })(this, document);
        ...
        function adSetInView(o, v, t) {
            if (o)
                adSetInV = o;
            if (v)
                adsSonarV = parseFloat(v);
            if (t)
                adsSonarT = parseInt(t);
        }
        function htmlAdWHDyn(m, s, t, dv, fn, ds) {
            htmlAdWH(m, '', '', t, dv, fn, ds, s.toString())
        }
        function htmlAdWH(m, w, h, t, dv, fn, ds, sz) {
            ...
            if (pr.indexOf('http') < 0)
                pr = 'http://';
            if (!adsVal)
                adsVal = adsGetValues();
            if (!adsChn && wi.s_265 && wi.s_265.channel)
                adsChn = 'kvoch=' + escape(wi.s_265.channel) + ';';
            if (t) {
                t = t.toLowerCase();
                if (t.indexOf('c') > 0) {
                    c = 1;
            ...
            if (adsTp == 'F' || t == 'ajax' || t == 'f')
                f = 1;
            if (t == 'text' || f) {
                if (!fn || fn == '')
                    fn = adsPage;
                if (fn == '' || (adsUA.indexOf('opera')>-1)) {
                    adsTp = 'J';
                    t = '', f = ''
                }
            }
            ...
            if (r == 0) {
                if (!adsNMSG && adsUA.indexOf('ipad')==-1)
                    adsCkPlg();
                if (!adsNMSG)
                    inc = 'artexc=art_flash,art_rrflash;';
                var s1 = '', s2 = '';
                if (sz&&!rr)
                    s2 = "allowedSizes=" + sz + ";"
                else if (ds != 'r')
                ...
                if (adsExcV == 'blank')
                    inc = 'artexc=all;'
                else if (adsExcV == 'imgOnly')
                    inc = 'artexc=all;artinc=art_image,art_img1x1,art_3pimg,art_rrimage,art_rrimg1x1,art_rr3pimg;';
                else if (adsExcV == 'noflash')
                    inc = 'artexc=art_flash,art_rrflash;'
                s1 = adsATOth.toLowerCase() + adsSrAT + adsATWM + adsVal + 'kvmn=' + m + ';kvgrp=' + adsScr + ';kvismob=' + adsDev + ';' + adsChn + "extmirroring=0;kvtile=" + adsTile + ";target=_blank;" + adsTzAT;
                ...
                if ((m.indexOf('-')>-1) && (/^[0-9a-fm\-]+$/i.test(m))) {
                    ...
                    try {
                        if (wi.devicePixelRatio)
                            sd = 'screendensity=' + wi.devicePixelRatio + ';';
                        if (wi.top !== wi.self)
                            inI = true;
                    ...
                    s += s1 + kf + swh + sd + f1 + f2 + 'optn=1;random=' + adsScr;
                } else {
            ...
            if (t == 'text') {
                ...
                if (!adsSyncDelay || adsDivs.length == 1 || adsLoadSync)
                    ...
                    if (!adsSyncTime)
                        adsSyncTime = setTimeout(adsLoadedSync, adsSyncDelay);
                }
            } else if (t == 'iframe') {
                ...
                var ai;
                if (adsTp == 'A0')
                    ai = d.getElementById('adsF0')
                else 
                    ai = d.getElementById('adsF1');
                adsD = new Date();
