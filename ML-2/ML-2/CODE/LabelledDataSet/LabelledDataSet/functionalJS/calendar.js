...
calendar = {
    ...
    makeCalendar: function (year, month, day) {
        ...
        for (var i = 2010; i < current_year + 10; i++) {
            this.wrt("<option value='" + i + "'");
        ...
        this.wrt("<div class='calendar-cancel''><span onclick='calendar.hideCalendar();'>[X]</span></div>");
...
function setWeek(week) {
    document.calendarForm.week.value = week;
    document.calendarForm.submit();
}

function setSort(sort) {
    document.calendarForm.sort.value = sort;
    document.calendarForm.submit();
}

function doFilter() {
    var filterValues = ["eur", "usd", "jpy", "gbp", "chf", "aud", "cad", "nzd", "cny",
    "high", "medium", "low"];

    for (var i = 0; i < filterValues.length; i++) {

        if (document.forms.filterForm.elements[filterValues[i]].checked) {

            document.calendarForm.elements[filterValues[i]].value = "true";
        }
        else {
            document.calendarForm.elements[filterValues[i]].value = "false";
        }
    }
    document.calendarForm.submit();
}