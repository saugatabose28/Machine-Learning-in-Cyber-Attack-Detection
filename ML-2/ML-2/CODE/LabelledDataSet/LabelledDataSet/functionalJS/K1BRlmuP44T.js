...
__d("SystemEvents", ["Arbiter", "ErrorUtils", "SystemEventsInitialData", "UserAgent_DEPRECATED", "copyProperties", "setIntervalAcrossTransitions"], function(a, b, c, d, e, f, g, h, i, j, k, l) {
    ...
    function x() {
        ...
        w = y;
        if (aa)
            m.inform(m.TIME_TRAVEL, z);
        return aa;
    }
    n.push(x);
    n.push(function() {
        if (window.onerror != h.onerror)
            window.onerror = h.onerror;
    });
    k(m, {
...
__d("ChannelTransport", ["copyProperties", "bind", "AjaxRequest", "URI", "JSLogger", "DocRPC", "ChannelConstants", "setTimeoutAcrossTransitions"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n) {
    ...
    function r(z) {
        ...
        if (z.sticky_token)
            aa.sticky_token = z.sticky_token;
    ...
    function s(z, aa, ba) {
        ...
        if (z.sticky_token)
            ga.sticky_token = z.sticky_token;
        if (z.sticky_pool)
        ...
        if (z.watchdog && z.watchdog.enabled)
            ga.wtc = z.watchdog.doSerialize();
        da = Date.now();
    ...
    function t(z) {
        ...
        if (z.is_offline) {
            ...
            aa.state = 'active';
        if (aa.state === z.lastPresenceState)
            return;
        z.lastPresenceState = aa.state;
        if (z.profile)
            aa.profile = z.profile;
        if (z.capabilities)
    ...
    function u(z, aa, ba, ca) {
        ...
        if (!!z.watchdog && z.watchdog.enabled)
            fa.wtc = z.watchdog.doSerialize();
        if (z.uid && z.viewerUid) {
            fa.uid = z.uid;
            fa.viewer_uid = z.viewerUid;
        }
        if (z.sticky_token)
            fa.sticky_token = z.sticky_token;
        if (z.sticky_pool)
            fa.sticky_pool = z.sticky_pool;
        if ('trace_id' in z)
            fa.traceid = z.trace_id;
        var ga=!!z.send_active_when_offline;
        if (ga && z.is_offline) {
            fa.state = 'offline';
        } else if (z.userActive > 0 && ea < 60)
            fa.state = 'active';
        ...
        if (z.fantail_logs && z.fantail_logs.length > 0) {
            var ka = {};
            for (var la = 0; la < z.fantail_logs.length; la++)
                g(ka, z.fantail_logs[la]);
            ja.send(ka);
            z.fantail_logs = [];
        } else 
            ja.send();
    ...
    g(w.prototype, {
        ...
        enterState: function(z, aa) {
            ...
            if (z == 'ping') {
                ...
                this._request = u(aa, h(ba, '_processTransportData', aa.stateId), h(ba, 'exitState', {
                    status: m.OK,
                    stateId: aa.stateId
                }), h(ba, 'exitState', {
                    status: m.ERROR,
    ...
    g(x.prototype, {
        ...
        exitState: function(z) {
            if (this.manager.state == 'init' && z.status == m.OK)
    ...
    g(y.prototype, {
        ...
        enterState: function(z, aa) {
            ...
            if (z == 'ping') {
                ...
                this._request = u(aa, h(this, '_processTransportData', aa.stateId), h(this, 'exitState', {
                    status: m.OK,
                    stateId: aa.stateId
                }), h(this, 'exitState', {
                    status: m.ERROR,
...
__d("PresencePrivacy", ["Arbiter", "AsyncRequest", "ChannelConstants", "CurrentUser", "PresencePrivacyInitialData", "JSLogger", "PresenceUtil", "copyProperties"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n) {
    ...
    function pa(qa, ra) {
        ...
        if (sa.window_id === m.getSessionID())
            return;
        var ta = sa.data;
...
__d("PresenceCookieManager", ["Cookie", "CurrentUser", "Dcode", "ErrorUtils", "JSLogger", "PresenceInitialData", "PresenceUtil", "URI"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n) {
    ...
    function v() {
        try {
            ...
            if (s !== z) {
                ...
                t = null;
                if (z && z.charAt(0) == 'E')
                    z = i.decode(z.substring(1));
                if (z)
                    t = JSON.parse(z);
            }
            if (t && (!t.user || t.user === h.getID()))
                return t;
        } catch (y) {
    ...
    var x = {
        ...
        store: function() {
            ...
            for (var aa in r)
                z[aa] = j.applyWithGuard(r[aa], r, [y && y[aa]], function(ea) {
        ...
        isSupported: function() {
            ...
            try {
                if (g.ie() >= 11) {
                    ...
                    h=!!window.ActiveXObject&&!!new ActiveXObject("htmlfile");
            } catch (j) {
                h = false;
            }
            return h;
...
__d("VideoCallSupport", ["ActiveXSupport", "ChannelConstants", "MercuryConfig", "UserAgent"], function(a, b, c, d, e, f, g, h, i, j) {
    var k = {
        isSendWebrtcSupported: function() {
            return i.SendNewVCGK && k.isWebrtcSupported();
        },
        isReceiveWebrtcSupported: function() {
            return i.ReceiveNewVCGK && k.isWebrtcSupported();
        },
        isSkypeCallSupported: function() {
            if (j.isPlatform('Windows')) {
                if (j.isBrowser('IE >= 9') && j.isBrowserArchitecture('32')) {
                    return g.isSupported();
                } else 
                    return ((j.isBrowser('IE >= 7') && j.isBrowserArchitecture('32')) || j.isBrowser('Firefox >= 3.6') || j.isBrowser('Chrome >= 5') || j.isBrowser('Opera >= 12'));
            } else if (j.isPlatform('Mac OS X > 10.5'))
                return (j.isEngine('WebKit >= 500') || j.isBrowser('Firefox >= 3.6') || j.isBrowser('Chrome >= 5') || j.isBrowser('Opera >= 12'));
            return false;
        },
        isVideoCallSupported: function() {
            return (k.isSendWebrtcSupported() || k.isSkypeCallSupported());
        },
        isPluginInstalled: function(n) {
            if (n === 'undefined')
                n = true;
            var o = false;
            if (k.isSkypeCallSupported())
                if (l()) {
                    var p = null;
                    try {
                        p = new ActiveXObject('SkypeLimited.SkypeWebPlugin');
                        o=!!p;
                    } catch (q) {}
                    p = null;
                } else 
        ...
        getCapabilities: function() {
            var n = 0;
            if (k.isReceiveWebrtcSupported())
                n = n | h.CAPABILITY_VOIP_INTEROP;
            return n;
        }
    };
    function l() {
        return j.isBrowser('IE');
    }
    function m(n) {
        if (!navigator)
            return null;
        ...
        var o = navigator.mimeTypes['application/skypesdk-plugin'];
        return o && o.enabledPlugin;
    }
    e.exports = k;
}, null);
__d("ChannelManager", ["AjaxRequest", "Arbiter", "AsyncRequest", "ChannelConstants", "ChannelInitialData", "ChannelSubdomain", "ChannelTransport", "ChatVisibility", "DTSG", "Env", "FBAjaxRequest", "ISB", "JSLogger", "MessagingRealtimeConstants", "MovingStat", "PresenceCookieManager", "PresenceState", "PresenceUtil", "SystemEvents", "URI", "UserActivity", "VideoCallSupport", "copyProperties", "createArrayFrom", "setIntervalAcrossTransitions", "setTimeoutAcrossTransitions", "WebStorage"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z, aa, ba, ca, da, ea, fa) {
    ...
    var ma = {
        ...
        getAccumulatedPullTimeSeconds: function() {
            ...
            if (this.pullStartTimeMillis > 0 && ya - this.pullStartTimeMillis <= na.LONGPOLL_TIMEOUT)
                xa += (ya - this.pullStartTimeMillis);
            var za = xa / 1000;
    ...
    function ta() {
        var xa = {};
        if (ia.getConfig('host'))
            xa[ia.getConfig('user_channel')] = ia.getConfig('seq', 0);
        return xa;
    }
    function ua() {
        var xa = Date.now(), ya = Date.now(), za = {
            total: 0
        }, ab = 'idle', bb = false;
        ...
        var db = function() {
            if (ia.isShutdown() || ia.shouldIdle())
                return;
            y.checkTimeTravel();
    ...
    function va() {
        ...
        function za() {
            fa(za, ya * 1000);
            var eb = ia.state;
            if (eb == 'idle' && ia.shouldIdle())
                return;
            ja.bump('conn_t', ya);
            if (eb == 'pull')
                ja.bump('conn_t_pull', ya);
        }
        za();
    ...
    ia = {
        ...
        init: function(xa) {
            ...
            if (typeof(aa) != 'undefined') {
                aa.subscribe(function() {
                    if (cb || n.isOnline())
                        na.userActive = Date.now();
                    if (!!na.web_sends_active_ping)
                        m.sendActivePing(na);
                }.bind(this));
            } else 
                ja.error('user_activity_undefined');
            v.register('ch', ta);
            var db = this.getConfig('max_conn', 2);
            na.subdomain = l.allocate(db);
            if (za && za.length && za.trim())
                na.sessionID = za;
        ...
        _sendIframeError: function(xa) {
            var ya = new i().setURI('/ajax/presence/reconnect.php').setData({
                reason: xa,
                fb_dtsg: o.getToken()
        ...
        _getDelay: function() {
            ...
            if (this.proxyDown && 'proxy_down_delay_millis' in na)
                xa = na.proxy_down_delay_millis;
            this.proxyDown = false;
            return Math.floor(xa * (1 + Math.random() * .5));
        },
        enterState: function() {
            if (this._inEnterState)
                ja.warn('enterstate_recursion');
        ...
        _enterState: function(xa) {
            ...
            if (xa.indexOf('reconnect') === 0)
                na.reconnectOverrideTimeMillis = Date.now();
            ...
            if (ab <= 0) {
                ...
                if (/init|idle|pull|ping/.test(this.state)) {
                    if (na.streamingCapable && /pull/.test(this.state))
                        this.heartbeats = [];
        ...
        exitState: function(xa, ya) {
            ...
            if (ab == j.SYS_TIMETRAVEL)
                oa.initialized = false;
            if (this.isShutdown() || za < pa)
                return;
            ...
            if (/ping/.test(cb) && ab != j.OK)
                db.url = m.getURI(na).toString();
            if (this.nextState)
                db.nextState = this.nextState;
            if (ya && ya.errorType) {
                na.lastRequestErrorReason = ya.errorType;
                db.xhr = ya.toJSON ? ya.toJSON() : ya;
                if (ya.errorType == g.PROXY_ERROR) {
                    this._logFantail('got 5xx http status code, setting long delay', j.FANTAIL_ERROR);
                    this.proxyDown = true;
                }
                delete db.xhr.json;
            } else if (ab != j.OK)
                na.lastRequestErrorReason = ab;
            if (ya && ya.json) {
                if (ya.json.t)
                    db.t = ya.json.t;
                if (ya.json.reason)
                    db.reason = ya.json.reason;
                if (ya.json.seq)
                    db.seq = ya.json.seq;
            }
            ja.log('exit_' + cb, db);
        ...
        _processTransportData: function(xa, ya) {
            ...
            case 'continue':
                ...
                wa(true);
                if (!na.inStreaming || na.STREAMING_EXIT_STATE_ON_CONTINUE)
                    this.exitState({
                        status: j.OK,
                        stateId: xa
                    });
                    ...
                    if (fb >= bb) {
                        ...
                        if (gb.type) {
                            if (gb.type === 'messaging') {
                                ...
                                if (gb.unread_counts && gb.unread_counts.inbox)
                                    h.inform(j.getArbiterType('pages_inbox_count_update'), {
                                        page_id: gb[t.VIEWER_FBID],
                                        inbox_unread: gb.unread_counts.inbox
                                    });
                            } else 
                                ja.debug('message', {
                                    type: gb.type
                                });
                                h.inform(j.getArbiterType(gb.type), {
                                    obj: gb
                                });
                        }
                    } else 
                        ja.warn('seq_regression', {
                            seq: fb,
                            last_seq: bb,
                            messages: eb.length
                        });
            ...
            case 'heartbeat':
                if (na.inStreaming) {
                    ...
                    if (this.heartbeats.length > 0) {
                        ...
                        ja.log('heartbeat_interval', {
                            client_id: na.sessionID,
                            interval: jb
                        });
                    }
        ...
        _enter_init: function() {
            if (qa.init >= ia.getConfig('MAX_INIT_FAILS', 2))
                return setTimeout(this.exitState.bind(this, {
                    status: j.ERROR_MAX,
                    stateId: pa
                }), 0);
            this._initTimer = fa(this.exitState.bind(this, {
                status: j.ERROR,
                stateId: pa
            }, 'timeout'), na.IFRAME_LOAD_TIMEOUT);
        },
        _enter_reconnect: function(xa) {
            ...
            ab.onError = (function() {
                ...
                if (ab.errorType == g.TRANSPORT_ERROR || ab.errorType == g.PROXY_ERROR)
                    this._shutdownHint = j.HINT_CONN;
                if (bb && bb == 1356007) {
        ...
        _exit_init: function(xa) {
            ...
            if (xa == j.ERROR_MAX)
                this._sendIframeError(j.reason_IFrameLoadGiveUp);
        },
        _exit_pull: function(xa, ya) {
            var za = xa == j.OK;
            oa.reportPullReturned(za, this);
            if (za) {
                this.lastPullTime = Date.now();
            } else {
                var ab = "exit status: " + xa;
                if (ya && ya.errorType)
                    ab += " ajax request error: " + ya.errorType;
                this._logFantail('pull failed with status: ' + ab, j.FANTAIL_ERROR);
            }
        },
        _exit_ping: function(xa) {
            if (xa == j.OK) {
                ...
                this._logFantail('ping failed with status: ' + xa, j.FANTAIL_ERROR);
        },
        _reportProxyMisguidedMsg: function(xa, ya) {
            this._logFantail('misguided message to ' + ya + ' meant for ' + xa, j.FANTAIL_ERROR);
            var za = Date.now();
            if (za - this.lastReportOnMisguidedMsgTime <= na.CHANNEL_PROXY_REPORTING_MIN_INTERVAL)
                return;
            ...
            if (na.sticky_token)
                ab.sticky_token = na.sticky_token;
        ...
        _probeTest: function() {
            ...
            if (na.sticky_token)
                ya.sticky_token = na.sticky_token;
                ...
                if (bb && bb.json && bb.json.t === 'heartbeat') {
                    ...
                    if (xa.length >= 2) {
                        ...
                        if (db >= na.PROBE_HEARTBEATS_INTERVAL_LOW && db <= na.PROBE_HEARTBEATS_INTERVAL_HIGH) {
                            ...
                            this._logFantail('probe request failed due to heartbeat delay', j.FANTAIL_ERROR);
                        ja.log('probe_ok', {
                            time: db
                        });
                    }
            ...
            ab.onSuccess = function(bb) {
                if (xa.length != 2) {
                    na.streamingCapable = false;
                    ja.error('probe_error', {
                        error: 'beats.length = ' + xa.length
                    });
                }
            };
        ...
        _logFantail: function(xa, ya) {
            ...
            if (!na.fantail_enabled || na.fantail_logs.length > za)
                return;
    ...
    if (k.channelConfig) {
        ...
        if (/shutdown/.test(k.state))
            ia._shutdownHint = j[k.reason];
    ...
    function r() {
        ...
        if (this._requests < this._config.maxRequests) {
            ...
            if (this._config.clearOnQuicklingEvents) {
                ...
                this._handle = m(this.request.bind(this), t);
        } else 
            this._done();
    }
    function s() {
...
__d("PresenceStatus", ["AvailableListConstants", "BanzaiODS", "ChatVisibility", "CurrentUser", "LastMobileActiveTimes", "LogHistory", "PresencePrivacy", "ServerTime", "createObjectFrom"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o) {
    h.setEntitySample('presence', .0001);
    var p = l.getInstance('presence_status'), q = {}, r = {}, s = {}, t = {}, u = {}, v = {}, w = {}, x = {}, y = {
        ...
        getDetailedActivePresence: function(z) {
            ...
            if (ca === g.STATUS_ACTIVE || da === g.STATUS_ACTIVE) {
                ...
                if (!q[z]) {
                    p.error('inconsistent_presence', {
                        id: z,
                        presence: y.getDebugInfo(z)
                    });
...
__d("PresencePoller", ["AvailableListConstants", "AvailableListInitialData", "BanzaiODS", "ChannelConnection", "ChatContexts", "ChatVisibility", "CurrentUser", "JSLogger", "LastMobileActiveTimes", "Poller", "PresencePrivacy", "PresenceStatus", "ServerTime", "ShortProfiles", "UserActivity", "copyProperties", "debounceAcrossTransitions"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w) {
    ...
    fa.prototype.$PresencePollera = function(ga, ha, ia, ja, ka, la) {
        ...
        if (!Array.isArray(ia)) {
            r.resetPresenceData();
            for (var ma in ia)
                r.set(ma, ia[ma].a, false, ga, ia[ma].c, ia[ma].p);
        }
        if (ja)
            o.update(ja);
        if (ka)
            r.setMobileFriends(ka);
        if (la)
            r.setBirthdayFriends(la);
    ...
    fa.prototype.$PresencePollerd = function(ga) {
        ...
        if (ga.getError() == 1356007)
            return;
        this.$PresencePoller9++;
        if (this.$PresencePoller9 >= x)
...
__d("AvailableList", ["Arbiter", "ArbiterMixin", "AsyncRequest", "AvailableListConstants", "BanzaiODS", "ChannelConnection", "ChannelConstants", "ChatConfig", "ChatVisibility", "JSLogger", "LastMobileActiveTimes", "PresencePoller", "PresencePrivacy", "PresenceStatus", "ServerTime", "ShortProfiles", "TypingStates", "copyProperties", "debounceAcrossTransitions", "emptyFunction"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z) {
    ...
    function da(ja) {
        ...
        for (var la in ka)
            ca(la, ka[la].a, true, 'mercury_tabs', ka[la].c, ka[la].p);
    }
    function ea() {
        ia.restart();
    }
    ...
    g.subscribe(m.getArbiterType('buddylist_overlay'), function(ja, ka) {
        var la = ka.obj.overlay;
        for (var ma in la)
            aa.set(ma, la[ma].a, la[ma].s || 'channel', la[ma].vc, la[ma].p);
    });
...
__d("ChatImpressionLogger", ["AsyncSignal", "requireWeak", "ChatConfig", "ChatVisibility", "Poller", "PresencePrivacy", "PresenceStatus", "debounceAcrossTransitions", "copyProperties"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o) {
    ...
    var u = {
        init: function(v) {
            ...
            if (w) {
                var x = i.get('periodical_impression_logging_config.interval'), y = new k({
                    interval: x,
                    setupRequest: t,
                    clearOnQuicklingEvents: false,
                    dontStart: true
        ...
        logImpression: function(v, w, x) {
            var y = i.get('chat_impression_logging_with_click'), z = {
...
__d("ChatOpenTab", ["Event", "requireWeak", "ChatWelcomeMessage", "ContextualThing", "DOM", "csx", "cx", "MercuryIDs", "Parent", "curry"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {
    ...
    var s = 716, t = 'messaging_tracking';
    function u() {
    ...
    function v(aa) {
        d(['LogHistory', 'MercuryThreads', 'WebMessengerPermalinkConstants', 'goURI'], function(ba, ca, da, ea) {
            ca.get().getThreadMeta(aa, function(fa) {
                if (q && q.isInitialized()) {
                    q.tabController.openTab(aa, q.tabsViewport);
                } else 
                    ea(da.getURIPathForThreadID(aa));
                if (!z.canOpenTab())
                    ba.getInstance('mercury').error('Unable to open chat tab', fa);
            });
        });
        if (document.documentElement.clientHeight <= s)
            u();
    }
    ...
    var z = {
        ...
        openThread: function(aa, ba, ca) {
            d(['MercuryServerRequests'], function(da) {
                if (n.isValid(aa)) {
                    v(aa);
                } else 
                    da.get().getClientThreadID(aa, v);
                x(ba, aa, null, ca);
                u();
            });
        },
        ...
        openPageTab: function(aa, ba, ca) {
            d(['MercuryThreads'], function(da) {
                ...
                da.get().getThreadMeta(ea, function(fa) {
                    if (aa && aa.length > 0) {
                        var ga = (Date.now() - fa.timestamp) / 1000, ha = ga / 3600;
                        if (fa.message_count === 0 || ha > 24)
                            i.setWelcomeMessage(ea, n.getParticipantIDFromUserID(ba), aa);
                    }
                });
                v(ea);
                x(ca, ea, ba);
        ...
        listenOpenUserTab: function(aa, ba, ca) {
            if (!y(aa))
                w(aa, ba, ca, z.openUserTab);
        },
        listenOpenPageTab: function(aa, ba, ca, da) {
            if (!y(aa))
                w(aa, ba, da, p(z.openPageTab, ca));
        },
            ...
            d(['ChatTypeaheadConstants', 'MercuryParticipantTypes'], function(da, ea) {
                if (ba === da.THREAD_TYPE) {
                    if (aa) {
                        ...
                        z.openEmptyTab(null, ca);
                } else if (!ba || ba === ea.FRIEND || ba === da.FRIEND_TYPE || ba === da.USER_TYPE)
                    z.openUserTab(aa, ca);
            });
        }
    };
