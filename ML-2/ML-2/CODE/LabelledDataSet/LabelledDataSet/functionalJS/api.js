...
function OptinMonster() {
        ...
        if ( exit ) {
            ...
            if ( 'lightbox' == type || 'canvas' == type ) {
                ...
                if ( ! self.getCookie('om-' + id + '-closed') ) {
                    ...
                    // Display the optin.
                    $('#om-' + optin).css('bottom', '-' + $('#om-' + optin).outerHeight() + 'px').show().animate({
                        'bottom' : parseInt(position) 
                    }, 300, function() {
                        // Set the visibility of the optin to open.
                        OptinMonsterOptins[self.getProp('optin_js')].visible = true;
    ...
    this.loading = function(target) {
        ...
        $('#om-' + self.getProp('optin')).find('.optin-monster-loading').css({
            width: width,
            height: height,
            top: position.top + margin,
            left: position.left,
            background: 'url(' + self.getProp('preloader') + ') no-repeat 50% 50%',
            position: 'absolute',
            zIndex: 84736365452,
            backgroundSize: '20px' 
        });
    ...
    this.successMessage = function(target) {
        ...
        $('#om-' + optin).find('.optin-monster-success-overlay').css({
            width: width,
            height: height,
            top: position.top,
            left: position.left,
            ...
            opacity: 0,
            display: 'block' 
        }).append('<div class="optin-monster-success-message">' + message + '</div>');

        // Position the content inside of the div to be vertically centered.
        $('#om-' + optin).find('.optin-monster-success-message').css({
            'margin-top': (height - $('#om-' + optin).find('.optin-monster-success-message').height()) / 2 
        });
        ...
        $(window).resize(function() {
            ...
            $('.optin-monster-success-message').css({
                'margin-top': ($('#om-' + type + '-' + theme + '-optin').outerHeight() - $('.optin-monster-success-message').height()) / 2 
            });
        });
    },
