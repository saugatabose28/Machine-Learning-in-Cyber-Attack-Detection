var Scriptaculous = {
    ...
    load: function() {
        ...
        "undefined" || a(Prototype.Version) < a(Scriptaculous.REQUIRED_PROTOTYPE))
        ...
        $$("script[src]").findAll(function(c) {
            ...
            (c ? c[1] : "builder,effects,dragdrop,controls,slider,sound").split(",").each(function(e) {
    ...
    node: function(a, b, c) {
        ...
        if (f && f.tagName.toUpperCase() != a)
            f = f.getElementsByTagName(a)[0];
        f || (f = document.createElement(a));
        if (f) {
            if (b)
                if (this._isStringOrNumber(b) || b instanceof Array ||
                b.tagName)
                    this._children(f, b);
                else {
                    var g = this._attributes(b);
                    if (g.length) {
                        try {
                            d.innerHTML = "<" + a + " " + g + "></" + a + ">"
                        } catch (h) {}
                        ...
                        if (!f) {
                            ...
                            for (attr in b)
                                f[attr == "class" ? "className": attr] = b[attr]
                        }
        ...
        if (b.tagName)
            ...
            b.flatten().each(function(c) {
                if (typeof c == "object")
                    ...
                    Builder._isStringOrNumber(c) && a.appendChild(Builder._text(c))
                });
        else 
...
String.prototype.parseColor = function(a) {
    ...
    if (this.slice(0, 4) == "rgb(") {
        var c = this.slice(4, this.length - 1).split(","), d = 0;
        do 
...
var Effect = {
    ...
    Transitions: {
        ...
        spring: function(a) {
            return 1 -
            Math.cos(a * 4.5 * Math.PI) * Math.exp( - a * 6)
        },
        none: function() {
            return 0
        },
...
Effect.Base = Class.create({
    ...
    inspect: function() {
        ...
        for (property in this)
            Object.isFunction(this[property]) || a.set(property, this[property]);
...
Effect.Tween = Class.create(Effect.Base, {
    initialize: function(a, b, c) {
        ...
        var d = $A(arguments), e = d.last();
        d = d.length == 5 ? d[3] : null;
        this.method = Object.isFunction(e) ? e.bind(a) : Object.isFunction(a[e]) ? a[e].bind(a) : function(f) {
            a[e] = f
        };
...
Effect.Scale = Class.create(Effect.Base, {
    ...
    setDimensions: function(a, b) {
        ...
        if (this.options.scaleFromCenter) {
            var d = (a - this.dims[0]) / 2, e = (b - this.dims[1]) / 2;
    ...
    update: function(a) {
        this.element.setStyle({
            backgroundColor: $R(0, 2).inject("#", function(b, c, d) {
                return b +
                (this._base[d] + this._delta[d] * a).round().toColorPart()
            }.bind(this))
        })
    },
    finish: function() {
...
Effect.ScrollTo = function(a, b) {
    var c = b || {}, d = document.viewport.getScrollOffsets(), e = $(a).cumulativeOffset();
    if (c.offset)
        e[1] += c.offset;
    return new Effect.Tween(null, d.top, e[1], c, function(f) {
        scrollTo(d.left, f.round())
    })
};
Effect.Fade = function(a, b) {
    a = $(a);
    var c = a.getInlineOpacity(), d = Object.extend({
        from: a.getOpacity() || 1,
        to: 0,
...
Effect.Shake = function(a, b) {
    ...
    var c = Object.extend({
        distance: 20,
        duration: 0.5
    }, b || {}), d = parseFloat(c.distance), e = parseFloat(c.duration) / 10, f = {
        top: a.getStyle("top"),
        left: a.getStyle("left")
    };
                                        ...
                                        afterFinishInternal: function(l) {
                                            new Effect.Move(l.element, {
                                                ...
                                                afterFinishInternal: function(m) {
                                                    m.element.undoPositioned().setStyle(f)
                                                }
                                            })
                                        }
                                    })
...
Effect.SlideDown = function(a, b) {
    a = $(a).cleanWhitespace();
    var c = a.down().getStyle("bottom"), d = a.getDimensions();
    return new Effect.Scale(a, 100, Object.extend({
        scaleContent: false,
        scaleX: false,
    ...
    return new Effect.Move(a, {
        ...
        afterFinishInternal: function(j) {
            ...
            100, {
                ...
                afterFinishInternal: function(l) {
                    l.effects[0].element.undoClipping().undoPositioned().setStyle(d)
                }
            }, c))
        }
    })
...
Effect.Pulsate = function(a, b) {
    a = $(a);
    var c = b || {}, d = a.getInlineOpacity(), e = c.transition || Effect.Transitions.linear;
    return new Effect.Opacity(a, Object.extend(Object.extend({
        ...
        transition: function(f) {
            return 1 - e( - Math.cos(f * (c.pulses || 5) * 2 * Math.PI) / 2 + 0.5)
        }
    }))
};
...
Effect.Morph = Class.create(Effect.Base, {
    initialize: function(a, b) {
        ...
        if (Object.isString(c.style))
            if (c.style.include(":"))
                ...
                c.afterFinishInternal = function(e) {
    ...
    update: function(a) {
        for (var b = {}, c, d = this.transforms.length; d--;)
            b[(c = this.transforms[d]).style] =
            c.unit == "color" ? "#" + Math.round(c.originalValue[0] + (c.targetValue[0] - c.originalValue[0]) * a).toColorPart() + Math.round(c.originalValue[1] + (c.targetValue[1] - c.originalValue[1]) * a).toColorPart() + Math.round(c.originalValue[2] + (c.targetValue[2] - c.originalValue[2]) * a).toColorPart() : (c.originalValue + (c.targetValue - c.originalValue) * a).toFixed(3) + (c.unit === null ? "" : c.unit);
...
Effect.Transform = Class.create({
    ...
    play: function() {
        return new Effect.Parallel(this.tracks.map(function(a) {
            var b = a.get("ids"), c = a.get("effect"), d = a.get("options");
            return [$(b) || $$(b)].flatten().map(function(e) {
                return new c(e, Object.extend({
                    sync: true
                },
                d))
            })
...
Element.getStyles = document.defaultView && document.defaultView.getComputedStyle ? function(a) {
    ...
    a = $(a);
    var b = a.currentStyle, c;
    c = Element.CSS_PROPERTIES.inject({}, function(d, e) {
        d[e] = b[e];
        return d
    });
    if (!c.opacity)
        c.opacity = a.getOpacity();
    return c
};
Effect.Methods = {
...
var Droppables = {
    ...
    add: function(a, b) {
        ...
        if (c.containment) {
            c._containers = [];
            var d = c.containment;
            Object.isArray(d) ? d.each(function(e) {
                c._containers.push($(e))
            }) : c._containers.push($(d))
        }
        if (c.accept)
            c.accept = [c.accept].flatten();
    ...
    findDeepestChild: function(a) {
        ...
        for (i = 1; i < a.length; ++i)
            if (Element.isParent(a[i].element,
            deepest.element))
                deepest = a[i];
        return deepest
    },
    ...
    show: function(a, b) {
        if (this.drops.length) {
            var c, d = [], e = b._hitAdjust, f;
            f = e ? [a[0] + e[0], a[1] + e[1]] : a;
            var g = b._hitWidth;
            if (g)
                var h = [f[0] - g / 2, f[1]], k = [f[0] + g / 2, f[1]];
            this.drops.each(function(j) {
                if (j.element != b) {
                    var l = false;
                    if (g) {
                        if (Droppables.isAffected(h, b, j) || Droppables.isAffected(k, b, j))
                            l = true
                    } else if (Droppables.isAffected(f, b, j))
                        l = true;
                    if (l && (j.element == b.parentNode ||!Element.isParent(j.element, b)))
                        d.push(j)
                }
            });
            if (d.length > 0)
                c = Droppables.findDeepestChild(d);
            this.last_active && this.last_active != c && this.deactivate(this.last_active);
            if (c) {
                Position.within(c.element, f[0], f[1]);
    ...
    b) {
        var c = this, d = {
            ...
            reverteffect: function(e, f, g) {
                new Effect.Move(e, {
                    x: - g,
                    y: - f,
                    duration: Math.sqrt(Math.abs(f^2) + Math.abs(g^2)) * 0.02,
                    queue: {
                        scope: "_draggable",
                        position: "end"
                    },
                    afterFinish: function() {
                        delete c.delta;
                        Draggables.notify("afterRevert", c)
                    }
                })
            },
        ...
        if (!b || Object.isUndefined(b.endeffect))
            Object.extend(d, {
        ...
        d = Object.extend(d, b || {});
        this.element = $(a);
        if (d.handle && Object.isString(d.handle))
            this.handle = this.element.down("." + d.handle, 0);
        if (!this.handle)
            this.handle = $(d.handle);
        if (!this.handle)
            this.handle = this.element;
        if (d.scroll&&!d.scroll.scrollTo &&
        !d.scroll.outerHTML) {
            d.scroll = $(d.scroll);
            this._isScrollChild = Element.childOf(this.element, d.scroll)
        }
    ...
    currentDelta: function() {
        return [parseInt(Element.getStyle(this.element,
        "left") || "0"), parseInt(Element.getStyle(this.element, "top") || "0")]
    },
    initDrag: function(a) {
        if (!Event.isRightClick(a)) {
            Draggables.notify("beforeStart", this, a);
            if (!(!Object.isUndefined(Draggable._dragging[this.element]) && Draggable._dragging[this.element]))
                if (Event.isLeftClick(a)) {
                    var b = Event.element(a), c = b.tagName.toUpperCase();
                    if (!(c && (c == "OBJECT" || c == "EMBED" || c == "INPUT" || c == "SELECT" || c == "OPTION" || c == "BUTTON" || c == "TEXTAREA")))
                        if (!(b.hasClassName("editable-text") || b.up(".editable-text") || b.hasClassName("w-move") ||
                        b.up(".w-move") || b.hasClassName("w-delete") || b.up(".w-delete") || b.hasClassName("w-upgrade") || b.up(".w-upgrade") || b.hasClassName("element-move-copy") || b.up(".element-move-copy") || b.hasClassName("ui-resizable-handle") || b.up(".ui-resizable-handle"))) {
                            var d = [Event.pointerX(a), Event.pointerY(a)], e = this.element.cumulativeOffset();
                            this.offset = [0, 1].map(function(f) {
                                return d[f] - e[f]
                            });
                            Draggables.activate(this);
                            if (this.options.nativeMousedown)
    ...
    updateDrag: function(a, b) {
        ...
        var c = 0, d = 0;
        if (this._inScroll && this.options.scroll != window) {
            c = this.options.scroll.scrollLeft - this.originalScrollLeft;
            d = this.options.scroll.scrollTop - this.originalScrollTop
        }
        b[0] += c;
        b[1] += d;
        this.draw(b);
        ...
        if (this.options.scroll) {
            ...
            if (this.options.scroll == window)
                ...
                c = Position.page(this.options.scroll).toArray();
                c[0] += Position.deltaX;
                c[1] += Position.deltaY;
            ...
            d = [0, 0];
            if (b[0] < c[0] + this.options.scrollSensitivity)
                d[0] = b[0] - (c[0] + this.options.scrollSensitivity);
            if (b[1] < c[1] + this.options.scrollSensitivity)
                d[1] = b[1] - (c[1] + this.options.scrollSensitivity);
            if (b[0] > c[2] - this.options.scrollSensitivity)
                d[0] = b[0] - (c[2] - this.options.scrollSensitivity);
            if (b[1] > c[3] - this.options.scrollSensitivity)
                d[1] = b[1] - (c[3] - this.options.scrollSensitivity);
            this._inScroll&&+new Date > this._whenEnteredScroll + 1E3 && this.startScrolling(d)
        }
        Prototype.Browser.WebKit && window.scrollBy(0, 0);
        Event.stop(a)
    },
    ...
    endDrag: function(a) {
        ...
        Draggables.notify("afterEnd", this, a)
    },
    draw: function(a) {
        ...
        b[0] -= c[0];
        b[1] -= c[1];
        if (this.options.scroll && this.options.scroll !=
        window && this._isScrollChild) {
            b[0] -= this.options.scroll.scrollLeft - this.originalScrollLeft;
            b[1] -= this.options.scroll.scrollTop - this.originalScrollTop
        }
        c = [0, 1].map(function(e) {
            return a[e] - b[e] - this.offset[e]
        }.bind(this));
        if (this.options.snap)
            c = Object.isFunction(this.options.snap) ? this.options.snap(c[0], c[1], this) : Object.isArray(this.options.snap) ? c.map(function(e, f) {
                return (e / this.options.snap[f]).round() * this.options.snap[f]
            }.bind(this)) : c.map(function(e) {
    ...
    scroll: function() {
        var a = new Date, b = a - this.lastScrolled;
        this.lastScrolled = a;
        if (this.options.scroll == window)
            with (this._getWindowScroll(this.options.scroll)) {
    ...
    _prevParent: null,
    _whenEnteredScroll: 0,
    _updateParent: function() {
        var a = this.element, b = a.parentNode;
        if (!this._prevParent || this._prevParent != b) {
            var c = this.options.scroll, d = false;
            if (c) {
                var e = this._inScroll;
                d = Element.childOf(a, c);
                if (e === false && d) {
                    this._whenEnteredScroll =+ new Date;
                    this.originalScrollLeft = this.originalScrollTop = 0
                }
            }
            this._inScroll = d;
            this._prevParent = b
...
var SortableObserver = Class.create({
    ...
    options: function(a) {
        if (a = Sortable._findRootElement($(a)))
    ...
    create: function(a, b) {
        ...
        ["beforeStart", "onStart", "onDrag", "onEnd", "afterEnd", "afterRevert"].each(function(g) {
            if (c[g])
                d[g] =
                function(h) {
                    c[g](h.element)
        ...
        if (c.dropOnEmpty || c.tree) {
            Droppables.add(a, f);
            c.droppables.push(a)
        }(c.elements || this.findElements(a, c) || []).each(function(g, h) {
            var k = c.handles ? $(c.handles[h]): c.handle ? $(g).select("." + c.handle)[0]: g;
            c.draggables.push(new Draggable(g, Object.extend(d, {
                handle: k
            })));
            Droppables.add(g, e);
            if (c.tree)
        ...
        if (c.tree)(Sortable.findTreeElements(a, c) || []).each(function(g) {
            Droppables.add(g, f);
            g.treeNode = a;
            c.droppables.push(g)
        });
        this.sortables[a.identify()] =
        c;
        ...
        if (!(c > 0.33 && c < 0.66 && Sortable.options(b).tree))
            if (c > 0.5) {
                ...
                if (b.previousSibling != a) {
                    ...
                    if (Sortable._insertBefore(a, b, b.parentNode, Sortable.options(c).onBeforeChange)) {
                        b.parentNode !=
                        c && Sortable.options(c).onChange(a);
                ...
                if (d != a) {
                    c = a.parentNode;
                    a.style.visibility = "hidden";
                    if (Sortable._insertBefore(a, d, b.parentNode, Sortable.options(c).onBeforeChange)) {
    ...
    onEmptyHover: function(a, b, c) {
        var d = a.parentNode, e = Sortable.options(b);
        if (a.parentNode != b&&!Element.isParent(b, a)) {
            ...
            if (f) {
                var h = Element.offsetSize(b, e.overlap) * (1 - c);
                for (c = 0; c < f.length; c += 1)
                    if (h - Element.offsetSize(f[c], e.overlap) >= 0)
                        h -= Element.offsetSize(f[c], e.overlap);
                    else {
                        g = h - Element.offsetSize(f[c], e.overlap) / 2 >= 0 ? c + 1 < f.length ? f[c + 1] : null : f[c];
                        break
                    }
            }
            if (Sortable._insertBefore(a, g, b, Sortable.options(d).onBeforeChange)) {
                Sortable.options(d).onChange(a);
                e.onChange(a)
            }
        }
    },
    _insertBefore: function(a, b, c, d) {
        for (; ;) {
            var e = b ? b.previousSibling: c.lastChild;
            if (e && (e._isPlaceholder || e.nodeType !=
            1))
                b = e;
            else 
                break
        }
        if (a.nextSibling != b || a.parentNode != c) {
            d && d(a);
            c.insertBefore(a, b);
            return true
        }
    ...
    mark: function(a, b) {
        ...
        if (!(c&&!c.ghosting)) {
            ...
            if (b == "after")
                c.overlap == "horizontal" ? Sortable._marker.setStyle({
    ...
    sequence: function(a, b) {
        ...
        return $(this.findElements(a, c) || []).map(function(d) {
            return d.id.match(c.format) ?
            d.id.match(c.format)[1] : ""
        })
    },
    setSequence: function(a, b, c) {
        a = $(a);
        var d = Object.extend(this.options(a), c || {}), e = {};
        this.findElements(a, d).each(function(f) {
            if (f.id.match(d.format))
                e[f.id.match(d.format)[1]] = [f, f.parentNode];
    ...
    serialize: function(a, b) {
        a = $(a);
        var c = Object.extend(Sortable.options(a), b || {}), d = encodeURIComponent(b && b.name ? b.name : a.id);
        return c.tree ? Sortable.tree(a, b).children.map(function(e) {
            return [d +
            Sortable._constructIndex(e) + "[id]=" + encodeURIComponent(e.id)].concat(e.children.map(arguments.callee))
        }).flatten().join("&") : Sortable.sequence(a, b).map(function(e) {
...
Element.findChildren = function(a, b, c, d) {
    ...
    if (b)
        b = [b].flatten();
    var e = [];
    $A(a.childNodes).each(function(f) {
        if (f.tagName && f.tagName.toUpperCase() == d && (!b || Element.classNames(f).detect(function(g) {
            return b.include(g)
        })))
            e.push(f);
        if (c)(f = Element.findChildren(f, b, c, d)) && e.push(f)
        });
...
Control.Slider = Class.create({
    initialize: function(a, b, c) {
        var d = this;
        this.handles = Object.isArray(a) ? a.collect(function(e) {
            return $(e)
        }) : [$(a)];
        this.track = $(b);
        this.options = c || {};
        ...
        this.handles.each(function(e, f) {
            f = d.handles.length - 1 - f;
            d.setValue(parseFloat((Object.isArray(d.options.sliderValue) ? d.options.sliderValue[f] : d.options.sliderValue) || d.range.start), f);
            e.makePositioned().observe("mousedown", d.eventMouseDown)
        });
    ...
    getNearestValue: function(a) {
        if (this.allowedValues) {
            ...
            var b = Math.abs(this.allowedValues[0] -
            a), c = this.allowedValues[0];
    ...
    setValue: function(a, b) {
        ...
        if (this.initialized && this.restricted) {
            if (b > 0 && a < this.values[b - 1])
                a = this.values[b - 1];
            if (b < this.handles.length - 1 && a > this.values[b +
            1])
                a = this.values[b + 1]
        }
    ...
    startDrag: function(a) {
        if (Event.isLeftClick(a)) {
            if (!this.disabled) {
                this.active = true;
                var b = Event.element(a), c = [Event.pointerX(a), Event.pointerY(a)];
                if (b ==
                this.track) {
                    ...
                    for (; this.handles.indexOf(b)==-1 && b.parentNode;)
                        b = b.parentNode;
    ...
    draw: function(a) {
        var b = [Event.pointerX(a), Event.pointerY(a)], c = this.track.cumulativeOffset();
