...
setTimeout(function() {
    if (5 !== new Date().getDay()) {
        return;
    }
    sinaadToolkit.sio.loadScript("http://d5.sina.com.cn/litong/zhitou/adJs/monboxMedia_baiwan.js", function() {
        ...
        if (scheduleDate.check() && scheduleTime.check()) {
            Store.ready(function(st) {
                var timeOut = (typeof st.get('monboxTime') !== 'undefined' ? st.get('monboxTime') : (+new Date())),
                    check = (+new Date()) - timeOut;
                if (check > 4 * 60 * 60 * 1000) {
                    st.set('monbox', 1);
                }
                monboxShow = (typeof st.get('monbox') !== 'undefined' ? st.get('monbox') : 1);

                if (monboxShowLimit > monboxShow) {
                    var monbox = new sinaadToolkit.MonBoxMedia({});
                    monbox.show();
                    st.set('monbox', (monboxShow + 1));
                    st.set('monboxTime', +new Date());
                }
            });
        }
    });
