...
=====================================================*/
{
	...
	{						
		...
		ref_row == undefined ? ref_row=3 : ref_row=(ref_row+2);
...
=====================================================*/
{
	...
	for(var k=start_row; k<row.length; k++)
	{
			...
			for(var j=0; j<nchilds; j++)// this loop retrieves cell data
			{
				...
				if(SearchArgs[j]!="")
				{
					...
					{						
					    ...
					    var regexp;
					    var searchValue = "";
					    if (SearchArgs[j] != undefined) {
					        if (SearchArgs[j].indexOf(';') != -1) {
					            var items = SearchArgs[j].split(';');
					            for (str in items) {
					                if (items[str] != "") {
					                    if (ematch || fprops["col_" + j] == "select") regexp = new RegExp('(^)' + regexpEscape(items[str].toLowerCase()) + '($)', "gi");
					                    else regexp = new RegExp(regexpEscape(items[str].toLowerCase()), "gi");
		...
		{ 
            ...
            var rowCurr = row[k].firstElementChild || row[k].children[0]; //row[k].firstChild;
            var rowText = rowCurr.textContent || rowCurr.innerText;
            if (rowText != "Low" &&
                rowText != "Medium" &&
                rowText != "High" &&
                rowText != "" &&
                previousDate != rowText) {
                previousDate = rowText;
                if (row[k].firstElementChild) {
                    var element = row[k].firstElementChild.firstElementChild;
                } else {
                    var element = row[k].firstChild.firstChild;
                }

//                if (element.innerHTML.indexOf("style") != -1 && element.innerHTML.indexOf(";") != -1) {
//                    element.innerHTML = element.innerHTML.substring(0, element.innerHTML.indexOf("style")) + element.innerHTML.substring(element.innerHTML.indexOf(";")+1);
//                    row[k].setAttribute("modifiedByFilter", "true");
                //                }
                if (element != null) {
                    if (element.innerHTML.indexOf("none") != -1) {
                        element.innerHTML = element.innerHTML.replace("none", "block");
                        row[k].setAttribute("modifiedByFilter", "true");
                    }
                }
            }
...
//function getGroupByDateCell(tr) {

//$("#div[id=""] span:first-child"
//}
	...
	if( TF_HasGrid(id) )
	{
		...
		{			
			...
			{ 
				...
				if (row[j].hasAttribute("modifiedByFilter")) {
				    try {
				        row[j].firstElementChild.firstElementChild.firstElementChild.style.display = "none";
				    } catch (ex) {
				        //row[j].firstChild.firstChild.firstChild.style.display = "none"; 
				        row[j].children[0].children[0].children[0].style.display = "none"; 
				    }
                }
