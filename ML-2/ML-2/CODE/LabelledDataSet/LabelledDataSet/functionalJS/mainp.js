...
(function(c, l) {
    ...
    c.widget("ui.dialog", {
        ...
        _create: function() {
            ...
            c("<div></div>").addClass("ui-dialog-title").attr("id", e).html(d).prependTo(f);
...
(function(c) {
    ...
    c.effects.size = function(b) {
        return this.queue(function() {
            var a = c(this), e = ["position", "top", "bottom", "left", "right", "width", "height", "overflow", "opacity"], g = ["position", "top", "bottom", "left", "right", "overflow", "opacity"], h = ["width", "height", "overflow"], i = ["fontSize"], f = ["borderTopWidth", "borderBottomWidth", "paddingTop", "paddingBottom"], k = ["borderLeftWidth", "borderRightWidth", "paddingLeft", "paddingRight"], p = c.effects.setMode(a, b.options.mode || "effect"), n = b.options.restore || false, m = b.options.scale || "both", l = b.options.origin, j = {
                height: a.height(),
                width: a.width()
            };
...
(function(D, $) {
    ...
    D.util = {
        truncate: function(html, length) {
            if (html) {
                var trunc = html.substr(0, length);
        ...
        mergeUrlParams: function(base, newParams) {
            var parts = base.split('?');
            var path = parts[0];
            var search = parts[1];
            var rawParams = (search && search.split('&')) || [];
            var params = {};
            $.each(rawParams, function(i, p) {
                var kv = p.split('=');
                params[kv[0]] = kv[1];
            });
            $.each(newParams, function(v, k) {
                params[v] = k;
            });
            search = $.map(params, function(val, key) {
                return key + '=' + val;
            }).join('&');
    ...
    $.extend($.fn, {
        moreless: function(maxHeight) {
            this.each(function(i, e) {
                ...
                var ml = $('<div></div>').addClass('moreless').attr('ml', 'more').css({
                    position: 'absolute',
                    bottom: 0,
                    left: 0,
                    right: 0,
...
(function(D, $) {
    D.api = {
        getRecentShareRecipients: function(success, error) {
            return $.ajax({
                'url': '/share/ws/recent',
                'type': 'GET',
                'dataType': 'json',
        ...
        getRecentStacks: function(username, linkList, success, error) {
            return $.get('/stacks/recent/' + username + '?urlm5s=' + linkList, success);
        },
        saveLink: function(params, callback) {
            return $.ajax({
                url: "/save",
                type: "POST",
                dataType: "json",
        ...
        deleteLink: function(url, callback) {
            return $.ajax({
                url: '/delete',
                type: 'POST',
                dataType: 'json',
        ...
        getLinkEditContent: function(url, stackId, suggesting_user, callback) {
            return $.ajax({
                url: '/save/confirm',
                data: {
                    url: url,
                ...
                success: callback,
                error: function() {
                    window.location.href = '/register?lo_action=save&next=' + encodeURIComponent('save?url=' + url);
                }
            });
        }
    };
...
(function(D) {
    ...
    D.EventEmitter.prototype = {
        ...
        once: function(event, callback) {
            if (this.otls[event]) {
                this.otls[event].push(callback)
            } else {
                this.otls[event] = [callback]
            }
        },
        ...
        emit: function(event) {
            ...
            var listeners = this.listeners[event];
            for (var i in listeners) {
                listeners[i].apply(null, a)
            }
            listeners = this.otls[event];
            for (i in listeners) {
                listeners[event].apply(null, a)
            }
            this.otls[event] = []
        }
    };
...
(function(D) {
    var cache = {};
    D.template = function(str) {
        var templateFunction = cache[str];
        if (!templateFunction) {
            var value = "var out = ''; out+=" + "'" +
            str.replace(/[\r\t\n]/g, " ").replace(/'(?=[^%]*%>)/g, "\t").split("'").join("\\'").split("\t").join("'").replace(/<%=(.+?)%>/g, "'; out += $1; out += '").split("<%").join("';").split("%>").join("out+='")
            + "'; return out;";
            templateFunction = cache[str] = new Function("t", value);
        }
...
(function($) {
    var actions = {
        encodeTag: function(tag) {
            return encodeURIComponent(tag.replace(/-/g, '_-_').replace(/ /g, "-").toLowerCase());
        },
        goToUserProfile: function(elem) {
            elem.click(function() {
        ...
        goToUserTag: function(elem) {
            elem.click(function() {
                var username = $('body').attr('username');
                window.location = '/' + username + '/' + actions.encodeTag(elem.attr('title'));
            });
        },
        goToTag: function(elem) {
            elem.click(function() {
                window.location = '/tag/' + actions.encodeTag(elem.attr('title'));
            });
        },
        addTag: function(elem) {
            elem.click(function() {
                var href = D.util.mergeUrlParams(window.location.href, {
                    addtag: actions.encodeTag(elem.attr('title'))
                });
                window.location = href;
            });
        },
        removeTag: function(target, elem) {
            target.click(function(evt) {
                evt.stopPropagation();
                var href = D.util.mergeUrlParams(window.location.href, {
                    removetag: actions.encodeTag(elem.attr('title'))
                });
                window.location = href;
            });
        },
        removeElement: function(target, elem) {
            target.click(function(evt) {
                evt.stopPropagation();
                elem.remove();
            });
        },
        moveTag: function(target, elem) {
            target.click(function(evt) {
                evt.stopPropagation();
                elem.remove();
            });
    ...
    $.extend($.fn, {
        tag: function() {
            this.each(function(i, elem) {
                elem = $(elem);
                var remAction = elem.attr('rem'), action = elem.attr('a');
                if (actions[action]) {
                    actions[action](elem, elem);
                }
                if (remAction == 'True') {
                    actions.removeTag(elem.find('[a=rem]'), elem);
                } else if (actions[remAction]) {
                    actions[remAction](elem.find('[a=rem]'), elem);
                }
            });
            return this;
        }
...
(function($) {
    ...
    var methods = {
        insertDates: function() {
            var dateTemplate = D.template($('#dateMarkerTempl').html())
            var currentDate = new Date(0);
            var now = new Date().getTime();
            var nowLimit = 1000 * 60 * 60 * 24;
            var recentAvailable = true;
            this.each(function(i, e) {
                var elem = $(e);
                var date = new Date(elem.attr('date') * 1000);
                if (recentAvailable && now - date.getTime() < nowLimit) {
                    elem.before(dateTemplate('Recent'));
                    ...
                    elem.before(dateTemplate(months[date.getMonth()] + ' ' + date.getDate() + ', ' + date.getFullYear()));
                }
                currentDate = date;
            });
        }
...
(function(D, $) {
    var EventEmitter = D.EventEmitter, modal = null;
    var defaultOptions = {
        marginTop: 50,
        width: 460,
        fadeTime: 100
    ...
    var Modal = function(elem) {
        this.element = $(elem);
        this.currentEmitter = new EventEmitter();
    ...
    Modal.prototype = {
        init: function(options) {
            ...
            this.win = $(window);
            if (this.titleNode) {
                this.titleNode.text(this.options.title);
                this.iconNode.attr('class', 'icon').addClass(this.options.icon);
                this.modalNode.attr('class', 'modal').addClass(this.options.cls);
                this.modalNode.css('width', this.options.width);
                this.actionsNode.empty();
                this.contentNode.empty();
                this._renderActions(this.options.actions);
                this.appendContent(this.options.content);
            }
            return this.currentEmitter;
        },
        show: function() {
            this.win.bind('resize.modal', this._centerModal.bind(this));
            this.element.fadeIn(this.options.fadeTime, function() {
                this.currentEmitter.emit('shown', this._getInputElems());
            }.bind(this));
            this._centerModal();
        },
        hide: function() {
            this.win.unbind('resize.modal');
            this.element.fadeOut(this.options.fadeTime, function() {
                this.currentEmitter.emit('hidden', this._getInputElems());
            }.bind(this));
        },
        insertAfter: function(contents, selector) {
            var html = this._buildContent(contents);
            this.contentNode.find(selector).after(html);
        },
        appendContent: function(contents, selector) {
            var html = this._buildContent(contents);
            if (selector) {
                this.contentNode.find(selector).append(html);
            } else {
                this.contentNode.append(html);
            }
        },
        prependContent: function(contents, selector) {
            var html = this._buildContent(contents);
            if (selector) {
                this.contentNode.find(selector).prepend(html);
            } else {
                this.contentNode.prepend(html);
            }
        },
        _centerModal: function() {
            var top = this.options.marginTop + this.win.scrollTop();
            var left = (this.win.width() - this.modalNode.outerWidth()) / 2;
            this.modalNode.css('left', left);
            this.modalNode.css('top', top);
        },
        _buildContent: function(contents) {
            var html = '';
            if (contents instanceof Array) {
                for (var i in contents) {
                    var data = contents[i]
                    html += this[data.type + 'Templ'](data);
                }
            } else {
                html = contents;
            }
            html = $(html);
            this._externalActions(html);
            return html;
        },
        _renderActions: function(actions) {
            var html = '';
            for (var i in actions) {
                html += this.actionTempl(actions[i]) + ' ';
            }
            html = $(html);
            this._externalActions(html);
            this.actionsNode.append(html);
        },
        _externalActions: function(node) {
            var self = this;
            node.filter('[a]').add(node.find('[a]')).each(function(idx, e) {
                e = $(e);
                var actions = e.attr('a').split(',');
                $.each(actions, function(i, action) {
                    var parts = action.split('|');
                    if (parts.length != 2) {
                        return;
                    }
                    e.bind(parts[0], function(evt, data) {
                        var inputs = self._getInputElems();
                        return self.currentEmitter.emit(parts[1], inputs, evt, data);
                    });
                });
                e.bind('keyup', function(evt) {
                    if (evt.which == 13) {
                        $(this).trigger('enter');
                    }
                });
        ...
        _initAttachPoints: function() {
            var self = this;
            this.element.find('[ap]').each(function(i, e) {
                e = $(e);
                var ap = e.attr('ap');
                self[ap] = e;
            });
        ...
        _initTemplates: function() {
            this.actionTempl = D.template($('#actionTempl').html());
            this.inputTempl = D.template($('#inputTempl').html());
            this.textTempl = D.template($('#textTempl').html());
            this.textareaTempl = D.template($('#textareaTempl').html());
            this.selectTempl = D.template($('#selectTempl').html());
            this.hiddenTempl = D.template($('#hiddenTempl').html());
        },
        _getInputElems: function() {
            var vals = {};
            this.element.find('input, textarea, select').each(function() {
                var elem = $(this);
                vals[elem.attr('name')] = elem;
            });
            return vals;
        }
    };
    D.modal = {
        ...
        init: function() {
            var emitter = modal.init.apply(modal, arguments);
            setTimeout(function() {
                D.modal.show()
            }, 0);
            return emitter;
        ...
        appendContent: function(content, selector) {
            modal.appendContent(content, selector);
        },
        prependContent: function(content, selector) {
            modal.prependContent(content, selector);
        },
        insertAfter: function(content, selector) {
            modal.insertAfter(content, selector);
        }
    };
})(D, jQuery);
var del = {
    ...
    DELETE_USER_PROMPT: 'Are you sure you want to deactivate your account? Your profile and links will be removed.',
    ...
    init: function() {
        $('[dropdown]').click(function(evt) {
            ...
            var elem = $(this);
            var moId = setTimeout(function() {
                elem.removeClass('open');
                elem.data('moId', false);
            }, 250);
            elem.data('moId', moId);
        }).mouseover(function() {
            var elem = $(this);
            var moId = elem.data('moId');
            if (moId) {
                clearTimeout(moId);
                elem.data('moId', false);
            }
        ...
        $('[dropdown] a').click(function(evt) {
            ...
            if (!parent.is('.open') || elem.is('.selected')) {
                evt.preventDefault();
                return;
            }
            parent.find('.selected').removeClass('selected');
            elem.addClass('selected');
            parent.trigger('change', elem.attr('value') || elem.text());
        });
        $('.pills .action .listView').click(function() {
            window.location.href = '/' + del.getLoggedInUsername();
        });
        $('.pills .action .hybridView').click(function() {
            window.location.href = '/' + del.getLoggedInUsername() + '?mode=hybrid';
        });
        $('.tooltip').qtip({
            position: {
                corner: {
        ...
        $('.stackThumbList li').live('hover', function() {
            var $this = $(this);
            var titleHeight = $this.find('h4').outerHeight();
            $(this).find('.stackOverlay').fadeToggle();
            $this.find('.stackOverlay p').css({
                'margin-top': titleHeight
            });
        });
        $('#singlePageTabs a').click(function() {
            var $this = $(this);
            var tabId = $this.attr("id").replace('Tab', 'Section');
            $this.siblings().removeClass('active');
            $this.addClass('active');
            $this.parents('.tabsPage').find('.tabSection').hide();
            $this.parents('.tabsPage').find('#' + tabId).show();
        });
        $('#feedSaveBtn, #feedStackemBtn, #feedShareBtn, #feedDeleteBtn').live("click", function() {
            ...
            if (checked_links.length > 0) {
                ...
                } else if (buttonId == 'feedShareBtn') {} else if (buttonId == 'feedDeleteBtn') {
        ...
        $('#global-searchinput').focus(function() {
            del.focusInputBoxCheckPlaceHolder($(this), 'search');
        });
        ...
        $('.link input:checkbox').change(function() {
            var bookmark = $(this).closest('.link');
            if ($(this).is(':checked')) {
        ...
        var bulkEditActions = $('#bulkEditActions');
        bulkEditActions.find('[a=delete]').click(function() {
            var md5s = del.getCheckedLinks();
            if (!md5s.length) {
                return del.okDialog('Check some links to delete.');
            }
            del.confirmDialog('Delete these links?', null, 'Delete', function() {
                del.deleteBookmarks(md5s.join());
            });
        });
        bulkEditActions.find('[a=public]').click(function() {
            var md5s = del.getCheckedLinks();
            if (!md5s.length) {
                return del.okDialog('Check some links to make public.');
            }
            del.confirmDialog('Make these links public?', null, 'Make Public', function() {
                del.convertBookmarkPrivacy(md5s.join(), true);
                del.uncheckAllBookmarks();
            });
        });
        bulkEditActions.find('[a=private]').click(function() {
            var md5s = del.getCheckedLinks();
            if (!md5s.length) {
                return del.okDialog('Check some links to make private.');
            }
            del.confirmDialog('Make these links private?', null, 'Make Private', function() {
                del.convertBookmarkPrivacy(md5s.join(), false);
                del.uncheckAllBookmarks();
            });
        });
        bulkEditActions.find('[a=createStack]').click(function() {
            var md5s = del.getCheckedLinks();
            if (!md5s.length) {
                return del.okDialog('Check some links to create a new stack.');
            }
            del.showCreateStack(md5s);
        });
        bulkEditActions.find('[a=addTag]').click(function() {
            var md5s = del.getCheckedLinks();
            if (!md5s.length) {
                return del.okDialog('Check some links to add tags to.');
            }
            del.showAddTagModal(md5s);
        });
        bulkEditActions.find('[a=renameTag]').click(del.showRenameTagModal);
        bulkEditActions.find('[a=removeTag]').click(function() {
            var md5s = del.getCheckedLinks();
            if (!md5s.length) {
                return del.okDialog('Check some links to remove tags from.');
            }
            del.showRemoveTagModal(md5s);
        });
        ...
        $('#username').keyup(function() {
            var field = this;
            var error_field = '#usernameValid';
            var error_message = 'Username should have at least 3 characters long and no more than 16 characters';
            var usernameValid = $(error_field);
            var username = $.trim(field.value);
            if (username && username != field.lastValue) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.error) {
                            usernameValid.html(del.ERROR_IMAGE + '<br/>' + data.error_msg);
                        } else {
        ...
        $('#emailAddress').keyup(function() {
            var field = this;
            var error_field = '#emailValid';
            var error_message = 'Please enter a valid email address';
            var validate_function = del.validateEmail;
            del.validateField(field, error_field, error_message, validate_function);
        });
        $('#password1').keyup(function() {
            var field = this;
            var error_field = '#passwordValid';
            var error_message = 'Password should be greater than 6 characters, less than 32 characters, and include at least one letter and one number or symbol';
            var validate_function = del.validatePassword;
            del.validateField(field, error_field, error_message, validate_function);
        });
        ...
        $('#register-form .nextStep').click(function() {
            $('#stepOne').hide();
            $('#stepTwo').fadeIn();
            return false;
        });
        $('#register-form .prevStep').click(function() {
            $('#stepTwo').hide();
            $('#stepOne').fadeIn();
            return false;
        });
        ...
        $('.introMessage .close').click(function() {
            $('.introMessage').fadeOut('fast');
        });
        $('#networkPrivacyForm input:checkbox, #email_url input:checkbox, #stack_order_options input:checkbox').change(function() {
            $('#ajaxLoading').show();
        ...
        $('.bookmark-actions .delete, .sidebarStats .delete').live('click', function() {
            ...
            del.confirmDialog('Are you sure you want to delete this link?', null, 'Delete', function() {
        ...
        $('#followStackBtn, #actionRemoveFromMyNetwork').mouseover(function() {
            ...
            if ($button.hasClass('following')) {
                $button.children('a:first').html('Unfollow').removeClass('green').addClass('large');
            }
        });
        $('#followStackBtn, #actionRemoveFromMyNetwork').mouseout(function() {
            var $button = $(this);
            if ($button.hasClass('following')) {
                $button.children('a:first').html('<span class="check"></span>Following').removeClass('large').addClass('green');
            }
        });
        ...
        $('#jcropTarget').Jcrop({
            boxWidth: 410,
            boxHeight: 305,
            onChange: del.updateJcropCoordinates,
            aspectRatio: 1,
            setSelect: [0, 0, 215, 215]
        });
        $('#delete_profile_pic_link').click(function() {
            del.confirmDialog('Are you sure you want to delete your profile pic?', '/settings/delete_profile_pic', 'delete', function(data) {
                if (data.error) {
        ...
        $('.exportComplete').click(function() {
            $(this).parents().find('.exportLinks').hide();
            $(this).parents().find('.exportCompleteMessage').fadeIn();
            return false;
        });
        ...
        $('#deleteAccount').click(function() {
            del.confirmDialog(del.DELETE_USER_PROMPT, '/settings/profile/delete', 'delete', function(data) {
                if (data.error) {
                    alert(data.error_msg);
                } else {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.error) {
                        ...
                        emitter = del.okDialog('settings saved!');
                        emitter.on('close', function() {
                            window.location.href = 'https://www.delicious.com/';
                        });
                    }
                },
                error: function(xhr) {
        ...
        del.initHomePage();
        del.initBanner();
        del.dropdown('#linkViewOptions');
        del.initSavePage();
    },
    getCheckedLinks: function() {
        var checkedLinks = $('.link input:checked, .post input:checked');
        var linkList = [];
        $.each(checkedLinks, function() {
            var bookmark = $(this).closest('.link, .post');
            var urlmd5 = bookmark.attr('md5');
            if (urlmd5) {
        ...
        $('textarea.tinymce').tinymce({
            ...
            setup: function(ed) {
                ed.onClick.add(function(ed) {
                    if (ed.getContent().indexOf(del.STACK_DESCRIPTION_PLACEHOLDER)>-1 || ed.getContent().indexOf(del.ADD_SLICE_PLACEHOLDER)>-1) {
                        ed.setContent('');
                    }
                });
            }
    ...
    followStack: function(username, stackId, $loadingElement, successFunc) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {
                'csrf_token': $loadingElement.attr('csrf_token')
            },
            cache: false,
            success: function(data) {
                if ($loadingElement) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (result == 'S') {
                            ...
                            $('#searchRelatedTags').html(relTagsInner.html()).find('[wt=tag]').tag();
                        }
                        $('[wt=tag]').tag();
                    }
                });
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error) {
                    ...
                    var emitter = del.okDialog('image is being processed, change will take effect in a few minutes.');
                    emitter.on('close', function() {
                        window.location.href = '/settings';
                    });
                }
    ...
    saveConfirmDialog: function(url, targetStackId) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $saveContainer.load('/save/confirm', confirmData, function(response, status) {
                    if (status != 'error') {
                        $('#saveTitle').focus();
                    }
                });
            ...
            error: function() {
                window.location.href = '/register?lo_action=save&next=' +
                encodeURIComponent('save?url=' + url);
            }
        });
    },
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error) {
                    ...
                    del.showCreateStack([data.urlmd5]);
    ...
    initBanner: function() {
        $('#userNav li.dropdown').not(".loggedOut").hoverIntent(function() {
            $(this).siblings().contents('ul').slideUp('fast', function() {
                $(this).parent().removeClass('selected');
            });
            $(this).contents('ul').slideDown('fast');
            $(this).addClass('selected');
        }, function() {
            $(this).contents('ul').slideUp('fast', function() {
                $(this).parent().removeClass('selected');
            });
        });
    },
    dropdown: function(loc) {
        if ($(loc + ' .default .selected a').length != 0) {
            var defaultText = $(loc + ' .default .selected a');
        } else {
            var defaultText = $(loc + ' .default .selected span');
        }
        defaultText.addClass('defaultText');
        defaultText.insertBefore(loc + ' .default .arrow');
        $(loc).contents('.default').hoverIntent(function() {
            $(this).siblings().contents('.expanding').slideUp('fast', function() {
                $().parent().removeClass('selected');
            });
            $(this).contents('.expanding').slideDown('fast');
            $(this).addClass('selected');
            $(this).contents('.expanding').contents('.element').hover(function() {
                $(this).addClass('hover');
            }, function() {
                $(this).removeClass('hover');
            });
        }, function() {
            $(this).contents('.expanding').slideUp('fast', function() {
                $(this).parent().removeClass('selected');
            });
        });
    },
    ...
    focusInputBoxCheckPlaceHolder: function($element, placeHolder) {
        var inputValue = $element.val();
        if (jQuery.trim(inputValue.toLowerCase()) == placeHolder) {
            inputValue = '';
        }
        $element.val(inputValue).parent().addClass('on');
    },
    blurInputBox: function($element, defaultText) {
        var value = $element.val();
        if ($.trim(value) == '') {
            $element.val(defaultText).parent().removeClass('on');
        }
    },
    submitLogin: function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.redirect) {
                    if (data.is_post) {
                        $.post(data.redirect, {}, function(data) {
                            if (data.error) {
                                del.okDialog(data.error_msg, 'Login Error');
                            }
                            window.location.href = '/' + username;
                        });
                    } else {
                    ...
                    del.okDialog(data.error_msgs, 'Login Error');
                } else {
                    window.location.href = '/' + username;
                }
            },
    ...
    submitRegister: function() {
        ...
        var response = $('input[name="recaptcha_response_field"]').val();
        var next = $('input[name="next"]').val();
        $.ajax({
            url: '/register',
            data: {
                ...
                if (data.error) {
                    for (field in data.error_fields) {
                        if (field != 'recaptcha_response_field')
                            $('input[name="' + data.error_fields[field] + '"]').prev().addClass('error');
                    }
                    alert(data.error_msgs);
                    ...
                    if (data.is_post) {
                        $.post(data.redirect, {}, function(data) {
                            ...
                            window.location.href = '/stacks/' + username;
                        });
                    } else {
                        window.location.href = data.redirect;
                    }
    ...
    validateField: function(field, error_field, error_message, validate_function) {
        var usernameValid = $(error_field);
        var username = $.trim(field.value);
        if (username && username != field.lastValue) {
            if (field.timer) {
            ...
            usernameValid.show().html(del.LOADING_IMAGE + ' checking...');
            field.timer = window.setTimeout(function() {
                if (!validate_function(username)) {
                    usernameValid.html(del.ERROR_IMAGE + '<br/>' + error_message);
                } else {
                    usernameValid.html(del.VALID_IMAGE);
                }
            }, 800);
    ...
    validatePassword: function(password) {
        return password.length > 5 && password.length < 32 && password.match(/\d+/);
    },
    validateEmail: function(email) {
        var emailReg = /^([A-Za-z0-9_\-\.\+])+@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
        return email.length > 2 && email.length < 255 && emailReg.test(email);
    },
    changePasswordSubmit: function() {
        ...
        if (oldpassword.length == 0 || newpassword.length == 0 || verifypassword.length == 0) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.result == true) {
                        emitter = del.okDialog('your password has been successfully changed.');
                        emitter.on('close', function() {
                            window.location.href = 'https://www.delicious.com/';
                        });
                    } else {
                        del.okDialog(data.error_msg, data.error);
                    }
                },
                error: function(xhr) {
                    del.displayError(xhr.responseText);
    ...
    twitter_disconnect: function(id) {
        del.confirmDialog('Are you sure you want to disconnect this account?', null, 'Disconnect', function() {
            url = 'settings/authorize/twitter?other_id=' + id + '&disconnect=true';
            window.location.href = url;
        }, 'risky');
    },
    set_twitter: function(id) {
        var url = 'settings/authorize/twitter?';
        if (id === undefined) {
            id = 'new_twitter_account';
        } else {
            url += 'other_id=' + id + '&';
        }
        var tc = 0;
        var load_tweets = $('#' + id + ' .load_tweets').attr('checked');
        var rt = $('#' + id + ' .load_retweets').is(':checked');
        var fav = $('#' + id + ' .load_favorite_tweets').is(':checked');
        var prv = $('#' + id + ' .keep_tweets_private').is(':checked');
        var import_tag = encodeURIComponent($('#' + id + ' .twitter_tag').val());
        if (!load_tweets) {
            tc = $('#' + id + ' input:radio:checked').val();
        }
        var params = '{"tc": ' + tc + ', "rt": ' + rt + ', "fav": ' + fav + ', "tag": "' + import_tag + '", "prv": ' + prv + '}';
        url += 'params=' + params;
        window.location.href = url;
    },
    disconnect_facebook: function(id) {
        del.confirmDialog('Are you sure you want to disconnect this Facebook account?', null, 'Disconnect', function() {
            url = 'settings/authorize/facebook?other_id=' + id + '&disconnect=true';
            window.location.href = url;
        }, 'risky');
    },
    set_facebook: function(id) {
        var url = 'settings/authorize/facebook?';
        if (id === undefined)
            id = 'new_facebook_account';
        else
            url += 'other_id=' + id + '&';
        var privacy = $('#' + id + ' input:radio:checked').val();
        var tags = encodeURIComponent($('#' + id + ' .facebook_tags').val());
        var params = '{"privacy": "' + privacy + '", "tag": "' + tags + '"}';
        url += 'params=' + params;
        window.location.href = url;
    },
    set_linkedin: function(id) {
        var url = 'settings/authorize/linkedin?';
        if (id === undefined) {
            id = 'new_linkedin_account';
        } else {
            url += 'other_id=' + id + '&';
        }
        var privacy = $('#' + id + ' .keep_linkedin_private').is(':checked');
        var import_tag = encodeURIComponent($('#' + id + ' .linkedin_tag').val());
        var params = '{"tag": "' + import_tag + '", "privacy": ' + privacy + '}';
        url += 'params=' + params;
        window.location.href = url;
    },
    disconnect_linkedin: function(id) {
        del.confirmDialog('Are you sure you want to disconnect this LinkedIn account?', null, 'Disconnect', function() {
            url = 'settings/authorize/linkedin?other_id=' + id + '&disconnect=true';
            window.location.href = url;
        }, 'risky');
    },
    updateNotificationOptionsSubmit: function() {
        var stack_comment = $('#notification_options #stack_comment').is(':checked');
        var follow = $('#notification_options #follow').is(':checked');
        var follow_stack = $('#notification_options #follow_stack').is(':checked');
        var follow_stack_update = $('#notification_options #follow_stack_update').is(':checked');
        var share = $('#notification_options #share').is(':checked');
        $.ajax({
            url: '/settings_notifications',
            type: 'POST',
            data: {
                'stack_comment': stack_comment,
                'follow': follow,
                'follow_stack': follow_stack,
                'follow_stack_update': follow_stack_update,
                'share': share
            },
            success: function(data) {
                emitter = del.okDialog('your notifications options been successfully updated.');
                emitter.on('close', function() {
                    window.location.href = 'https://www.delicious.com/';
                });
            },
    ...
    createStack: function(stackTitle, elementsJson, description, successFunction, showLoading, stackurl) {
        if (showLoading) {
            $('#delLoading').show();
        };
        var data = {
    ...
    callUpdateStack: function(stackId, stackTitle, stackDescription, elementsJson, stackThumb, category_id, penname_id, successFunction) {
        var data = {};
        if (stackTitle) {
    ...
    callPostComment: function(stackId, text, suggested_url, element_md5, successFunction) {
        var data = {};
        if (text) {
            data.text = text;
        }
        if (element_md5) {
            data.element_md5 = element_md5;
        }
        if (suggested_url) {
            data.suggested_url = suggested_url;
        }
        del.showLoadingModal();
        $.ajax({
            url: '/stacks/comments/add/' + stackId,
            type: 'POST',
            data: data,
            success: function(data) {
                if (data.error) {
                ...
                del_stack_view.loadComments(stackId, '', 20, 0);
            },
            error: function(xhr) {
                del.displayError(xhr.responseText);
            },
    ...
    callDeleteComment: function(stackId, comment_id, successFunction) {
        var data = {};
    ...
    shareStack: function(enc_stack_id, recipients, message, csrf_token, invite_collab) {
        $('#delLoading').show();
        var postData = {
            'enc_stack_id': enc_stack_id,
            'recipients': recipients,
            'csrf_token': csrf_token,
            'invite_collab': invite_collab
        };
        if (message) {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.error && result.next) {
                    ...
                    if (invite_collab) {
                        del.okDialog('your invitations have been sent!');
                    } else {
    ...
    displayError: function(errorText) {
        ...
        emitter.on('close', window.location.reload);
    },
    ...
    okDialog: function(sentence, title) {
        var emitter = D.modal.init({
        ...
        emitter.on('close', D.modal.hide);
        return emitter;
    },
    confirmDialog: function(question, url, successButtonName, successFunc, klass) {
        var emitter = D.modal.init({
            title: 'Confirm',
            content: [{
                type: 'text',
    ...
    checkAllBookmarks: function() {
        ...
        del.selectBookmark($('.link'));
    },
    uncheckAllBookmarks: function() {
        $("input[type='checkbox']").removeAttr('checked');
        del.deselectBookmark($('.link'));
    },
...
(function(del, $) {
    $.extend(del, {
        showShareModal: function(evt) {
            evt.preventDefault();
            var $this = $(this);
            var url = $this.attr('href');
            var stack_id = $this.attr('stackid');
            var title = $this.attr('title');
            var invite_collab = $this.attr('invite_collab');
            ...
            var type = 'Stack shares';
            if (invite_collab) {
                type = 'Stack invites';
            } else if (stack_id === undefined) {
                type = 'Link shares';
            }
            ...
            var shareStackModal = {
                ...
                content: [{
                    ...
                    text: title,
                    cls: 'shareTitle',
                    faviconHost: D.util.parseUrl(url).host
                }, {
                    type: 'input',
                    field: 'recipients',
                    label: 'Type Emails or User Names (comma-separated)',
                    title: 'Tip: separate emails with commas'
                }, {
                    type: 'text',
                    text: type + ' via email will come from ' + email_address + ' so recipients know it\'s you and can reply.',
                    cls: 'email'
                }, {
                    type: 'textarea',
                    field: 'message',
            ...
            if (invite_collab) {
                shareStackModal.title = "Invite Others to Contribute";
            }
            D.api.getRecentShareRecipients(function(data) {
                if (!data.recent_shares.length)
                    return;
                var rrs = [{
                    type: 'select',
                    field: 'recentRecipients',
                    action: 'change|recentRecipientsChange',
                    options: [{
                        disabled: true,
                        text: 'Select from people recently shared with'
                    }
                    ].concat($.map(data.recent_shares, function(item, index) {
                        return {
                            text: item
                        };
                    }))
                ...
                D.modal.insertAfter(rrs, '.shareTitle');
            }, function(data) {
                console.log('error', data);
            });
            var emitter = D.modal.init(shareStackModal);
            emitter.on('cancel', D.modal.hide);
            emitter.on('shown', function(inputs) {
                inputs.recipients.focus();
            });
            emitter.on('recentRecipientsChange', function(inputs, evt) {
                inputs.recipients.val(inputs.recentRecipients.val());
            });
            emitter.on('share', function(inputs, evt) {
                var recipients = inputs.recipients.val();
                var message = inputs.message.val();
                var csrf_token = $('#csrf_token').val();
                if (!recipients) {
                    inputs.recipients.addClass('error').attr('placeholder', "recipients required")
                } else {
                    del.showLoadingModal();
                    if (stack_id && $.trim(stack_id).length > 0) {
                        del.shareStack(stack_id, recipients, message, csrf_token, invite_collab);
                    } else {
                        del.shareLink(url, null, recipients, message, csrf_token);
                    }
                }
            });
        },
        showSaveLink: function(evt) {
            evt.preventDefault();
            var elem = $(this);
            var url = elem.attr('href');
            var modal_title = "Save a Link"
            if (url) {
                return del.showEditModal(url, '_new_', modal_title, '');
            }
            var saveLinkModal = {
                title: "Save a Link",
                content: [{
                    ...
                    value: url,
                    label: 'Paste the URL',
                    action: 'enter|saveLink'
                }, {
                    type: 'text',
                    text: 'We also have a bookmarklet to help you save links.  Get it <a href="/tools" target="_blank">here</a>.'
                }
                ],
                actions: [{
                    text: 'Cancel',
                    ...
                    text: 'Save',
                    cls: 'green-txt',
                    action: 'click|saveLink'
                }
                ]
            };
            var emitter = D.modal.init(saveLinkModal);
            emitter.on('shown', function(inputs) {
                inputs.url.focus();
            });
            emitter.on('saveLink', function(inputs, evt) {
                if (!inputs.url.val() || inputs.url.val() == 'url required') {
                    inputs.url.addClass('error').val('url required');
                    return;
                }
                del.showEditModal(inputs.url.val(), '', "Save a Link", undefined);
            });
            emitter.on('cancel', D.modal.hide);
        },
        showRemoveTagModal: function(linkList) {
            del.showLoadingModal();
            $.ajax({
                ...
                success: function(data) {
                    if (data.duplicate_tags.length < 1) {
                        ...
                        var ce = del.okDialog("The following dialog lists duplicate tags among the chosen links to remove. Click 'x' on a tag to take the tag off the delete list - all remaining tags will be deleted.");
            ...
            var tagTemplate = D.template($('#tagTempl').html());
            function showModal(data) {
                var tagHtml = $($.map(data.duplicate_tags, function(t) {
                    return tagTemplate({
                        name: t,
                ...
                var modalConfig = {
                    title: 'The following tags will be removed:',
                    content: tagHtml,
                    actions: [{
                        text: 'Cancel',
                        action: 'click|cancel'
                ...
                var emitter = D.modal.init(modalConfig);
                emitter.on('cancel', D.modal.hide);
                emitter.on('remove', function(inputs, evt) {
                    var tags = $.map($('#modal [wt=tag]'), function(t) {
                        return $(t).attr('title')
                    });
                    if (tags.length < 1) {
                        return;
        ...
        showAddTagModal: function(linkList) {
            var tagTemplate = D.template($('#tagTempl').html());
            var modalConfig = {
                title: 'Add Tags',
                content: [{
                    type: 'input',
                    label: 'Type tags to add',
                    field: 'tags',
                    action: 'keydown|tagKey,blur|blurTag,autocompleteTag|autoTag'
                }
                ],
                actions: [{
                    text: 'Cancel',
            ...
            emitter.on('blurTag', function(inputs, evt, data) {
                setTimeout(function() {
                    addTag(inputs.tags.val());
                    inputs.tags.val('');
                }, 100);
            });
            emitter.on('autoTag', function(inputs, evt, data) {
                addTag(data);
                inputs.tags.val('');
            });
            emitter.on('tagKey', function(inputs, evt) {
                if (evt.which == 188 || evt.which == 13) {
                    evt.preventDefault();
                    addTag(inputs.tags.val());
                    inputs.tags.val('')
                }
            });
            emitter.on('add', function(inputs, evt) {
                var tags = $.map($('#modal [wt=tag]'), function(t) {
                    return $(t).attr('title')
                });
                if (tags.length < 1) {
            ...
            function addTag(tag) {
                if (tag&&!tag.match(/^\s+$/)&&!$('#modal [title="' + tag + '"]').length) {
                    D.modal.insertAfter($(tagTemplate({
                        name: tag,
                        removable: 'removeElement'
                    })).tag(), '[name=tags]');
                }
        ...
        showRenameTagModal: function() {
            del.showLoadingModal();
            $.get('/v1/json/tags/get?alpha=True', showModal);
            function showModal(data) {
                var modalConfig = {
                    title: 'Rename Tag',
                    content: [{
                        type: 'select',
                        label: 'Select a tag to rename',
                        field: 'oldTag',
                        options: $.map(data.tags, function(item, index) {
                            return {
                                text: item.tag.tag
                            };
                        })
                ...
                emitter.on('rename', function(inputs, evt) {
                    del.showLoadingModal();
                    $.post('/settings/tags/rename', {
                        old_tag: inputs.oldTag.val(),
                        new_tag: inputs.newTag.val()
                    }, function(data) {
                        if (data.error) {
                            del.okDialog(data.error_msg);
                        } else {
                            var ce = del.okDialog('Depending on system load, renaming can take several minutes to process across all bookmarks and tags.', 'Tag renaming does not apply immediately.');
                            ce.on('close', function() {
                                del.uncheckAllBookmarks();
                                window.location.href = '/' + del.getLoggedInUsername();
                            });
        ...
        showEditModal: function(url, stackId, title, suggesting_user) {
            D.api.getLinkEditContent(url, stackId, suggesting_user, function(content) {
                var emitter = del.showEditModalWithContent(content, title);
                emitter.on('saved', function() {
                    if (window.location.href.indexOf('stacks/view')==-1) {
                        window.location.reload();
                    };
                });
            })
        },
        showEditModalWithContent: function(content, title, options) {
            var tagTemplate = D.template($('#tagTempl').html());
            content = $(content);
            content.find('[wt=tag]').tag();
            var recoTags = content.find('[ap=recoTags]');
            var urlLabel = content.find('[for=url] span');
            var descCount = content.find('[ap=descCount]');
            var imageContainer = content.find('[ap=imageContainer]');
            var images = content.find('img');
            var currImageText = content.find('[ap=cImageNum]');
            var currImageIdx = 0;
            var currImageUrl;
            if (!recoTags.length ||!title) {
                title = 'Edit Link Info';
            }
            var modalConfig = {
                title: title || 'Edit Link Info',
                icon: 'edit',
                cls: 'editModal',
                width: 690,
                actions: [{
            ...
            if (!recoTags.length) {
                modalConfig.actions.unshift({
                    text: 'Delete',
                    cls: 'delete',
                    action: 'click|delete'
                });
            }
            $.extend(modalConfig, options || {});
            var emitter = D.modal.init(modalConfig);
            emitter.on('shown', function(inputs) {
                inputs.tags.focus();
                descCount.text(inputs.description.val().length);
                var priv = $('.modal .private').detach();
                $('.modal .m_actions').prepend(priv);
                if (inputs.noImage.is(':checked')) {
                    return;
                }
                var chosenImage = images.filter('[chosen=true]');
                if (!chosenImage.length) {
                    chosenImage = images.first();
                }
                currImageUrl = chosenImage.attr('url');
                imageContainer.prepend(chosenImage.detach());
                images = content.find('img');
                chosenImage.show();
            });
            emitter.on('save', function(inputs, evt) {
                var title = inputs.title.val();
                var oldUrl = inputs.savedUrl.val();
                var url = inputs.url.val();
                var tags = $.map($('#modal [ap=userTags] [wt=tag]'), function(t) {
                    return $(t).attr('title')
                }).join();
                var note = inputs.description.val();
                var isPrivate = inputs.private.is(':checked');
                var csrf_token = inputs.csrfToken.val();
                var noImage = inputs.noImage.is(':checked');
                del.showLoadingModal();
                var postParams = {
                    "url": url,
                    "oldUrl": oldUrl,
                ...
                if (!inputs.noImage.is(':checked')) {
                    postParams.chosen_image = currImageUrl;
                }
                D.api.saveLink(postParams, handleResponse);
                function handleResponse(error, data) {
                    if (error) {
                        D.modal.hide();
                        del.okDialog(error_msg);
                    } else if (data.rendered_link) {
                        emitter.emit('saved');
                        D.modal.hide();
                    } else {
            ...
            emitter.on('delete', function(inputs, evt) {
                var elem = $(evt.target);
                if (!elem.data('confirmed')) {
                    elem.data('confirmed', true);
                    elem.addClass('risky');
                    elem.html('Really Delete');
                    return;
                }
                del.showLoadingModal();
                D.api.deleteLink(inputs.url.val(), function(error, data) {
                    if (error) {
                        del.displayError(error);
                    } else {
            ...
            emitter.on('imageToggle', function(inputs, evt) {
                var image = $(images[currImageIdx]);
                if (inputs.noImage.is(':checked')) {
                    image.hide();
                } else {
                    image.show();
            ...
            emitter.on('nextImage', function(inputs, evt) {
                if (inputs.noImage.is(':checked')) {
                    return;
                }
                $(images[currImageIdx]).hide();
                if (++currImageIdx >= images.length) {
                    currImageIdx = 0;
                }
                currImageUrl = $(images[currImageIdx]).attr('url');
                currImageText.text(currImageIdx + 1);
                $(images[currImageIdx]).show();
            });
            emitter.on('prevImage', function(inputs, evt) {
                if (inputs.noImage.is(':checked')) {
                    return;
                }
                $(images[currImageIdx]).hide();
                if (--currImageIdx < 0) {
                    currImageIdx = images.length - 1;
                }
                currImageUrl = $(images[currImageIdx]).attr('url');
                currImageText.text(currImageIdx + 1)
                $(images[currImageIdx]).show();
            });
            emitter.on('urlEdit', function(inputs, evt) {
                var url = inputs.url.val();
                inputs.url.val('');
                inputs.url.slideToggle(100, function() {
                    inputs.url.val(url);
                });
            });
            emitter.on('blurTag', function(inputs, evt, data) {
            ...
            content.find('[ap=recoTags] [wt=tag]').click(function() {
                var tag = $(this).attr('title');
                addTag(tag);
                $(this).remove();
            });
            emitter.on('urlKeyup', function(inputs, evt) {
                urlLabel.text(inputs.url.val());
            });
            emitter.on('descKeyup', function(inputs, evt) {
                var desc = inputs.description.val();
                var length = desc.length;
                descCount.text(length);
                if (length > 1000) {
                    inputs.description.val(desc.substr(0, 1000));
                    descCount.text(1000);
                }
            });
            emitter.on('stackSelected', function(inputs) {
                if (inputs.stackId.val() == '_new_') {
                    stackName.show();
                } else {
                    stackName.hide();
                }
            ...
            emitter.on('cancel', D.modal.hide);
            function addTag(tag) {
                if (tag&&!tag.match(/^\s+$/)&&!$('#modal [ap=userTags] [title="' + tag + '"]').length) {
                    D.modal.prependContent($(tagTemplate({
                        name: tag,
                        removable: 'removeElement'
                    })).tag(), '[ap=userTags]');
                }
...
$(function() {
    var nav = $('#nav'), win = $(window), body = $('html,body'), con = $('#content');
    if (!con.length) {
        return;
    }
    win.bind({
    ...
    function alignNav() {
        var diff = con.offset().top - win.scrollTop();
        if (diff < 30) {
            nav.css({
                position: 'fixed',
                top: 60
    ...
    $('.question').click(function() {
        ...
        elem.next('.answer').slideToggle(100);
    });
    var anchor = decodeURIComponent(D.util.parseUrl(document.location.href).anchor);
    if (anchor)
        setTimeout(function() {
    ...
    function scrollToAnchor(anchor) {
        ...
        if (!target.length) {
            target = $('.question:contains("' + anchor + '")');
        }
        if (target.length) {
            body.animate({
    ...
    var existing = null;
    if ($('#toc').length == 0 && $('.mainContent').find('h3').length > 0) {
        $('.mainContent').before('<div id="toc">');
    } else {
        existing = $('#toc').find('li');
    }
    $('.mainContent').find('h3').each(function(i) {
        var current = $(this);
    ...
    $('body').find('h2').each(function(i) {
        var current = $(this);
        current.after('<div class="blueBreak"></div>');
    });
});
