  ...
  updateContainer: function() {
    ...
    if (this.maxHeight < this.list.offsetHeight) {
      ...
      if (shadow_height && !reversed) {
        ...
        setStyle(this.container, 'width', this.shadow.offsetWidth + 1); // compensate side border
      } else {
        hide(this.shadow);
      }
    }
...
function MDropdown(el, options) {
  if (this.constructor != MDropdown) {
    throw new Error('MDropdown was called without \'new\' operator');
  }
  ...
  this.items = this.options.items ? this.options.items : el.getAttribute('data-items');
  if (!this.items) {
  ...
  this.options.width = this.options.width || 130;

  var title = this.items[0][1];
  if (this.options.keepTitle) {
    title = el.innerHTML;
  }

  var arrowCls = this.options.withArrow ? 'mdd-arrow' : '';
  var iconCls = this.options.withIcon ? 'with-icon' : '';

  this.ddEl = ce('div', { innerHTML:
    '<div class="mdd-value ' + arrowCls + '">' + title + '</div>' +
    '<input type=text hidden name="' + (el.getAttribute('name') || el.id) + '" />' +
    '<div class="mdd-popup" style="display: none"></div>'
  , className: 'mdd-wrap ', id: el.id });

  var classes = el.classList;
  for (var i = 0; i < classes.length; i++) {
    addClass(this.ddEl, classes[i]);
  }
  this.ddEl.setAttribute('data-action-hash', el.getAttribute('data-action-hash')); // temp hack

  if (this.options.rtl) {
    addClass(this.ddEl, "mdd-rtl");
  }

  el.parentNode.replaceChild(this.ddEl, el);

  var matches = function(el, selector) {
    return (el.matches || el.matchesSelector || el.msMatchesSelector || el.mozMatchesSelector || el.webkitMatchesSelector || el.oMatchesSelector).call(el, selector);
  };

  this.itemsWrap = geByClass1('mdd-popup', this.ddEl);
  this.valueEl = geByClass1('mdd-value', this.ddEl);
  this.itemsContentEl = geByClass1('mdd-items-content', this.ddEl);
  this.hiddenInput = geByTag1('input', this.ddEl);

  setStyle(this.itemsWrap, 'width', this.options.width);

  MDropdown.prototype._onClick = function(event) {
    if (isVisible(this.itemsWrap)) return;

    if (MDropdown._currMDD) {
      MDropdown._currMDD.hide();
      MDropdown._currMDD = null;
    }

    var elPos = getXY(this.ddEl);
    var scrollTop = (window.pageYOffset || document.documentElement.scrollTop)  - (document.documentElement.clientTop || 0);
    var openUp = (elPos[1] + 190 - scrollTop) > window.innerHeight;

    var selectedHtml = '<div class="mdd-item-selected-wrap ' + iconCls + '"><div class="mdd-item-selected ' + arrowCls + '" id="' + this.items[0][0] + '">' + title + '</div></div>';
    var itemsHTML = '<div class="mdd-items-wrap"><div class="mdd-items-content">';
    for (var i = 0; i < this.items.length; i++) {
      var iconEl = '<div class="mdd-item-icon" id="' + this.items[i][0] + '"></div>';
      itemsHTML += '<div class="mdd-item ' + iconCls + '" id="' + this.items[i][0] + '">' + iconEl + '<div class="mdd-item-name">' + this.items[i][1] + '</div></div>';
    }
    itemsHTML += '</div> <div class="mdd-cascade-wrap" style="display: none"></div> </div>';

    if (openUp) {
      itemsHTML = itemsHTML + selectedHtml;
    } else {
      itemsHTML = selectedHtml + itemsHTML;
    }

    this.itemsWrap.innerHTML = itemsHTML;

    var itemsContentEl = geByClass1('mdd-items-content', this.itemsWrap);
    this.itemsCascadeWrap = geByClass1('mdd-cascade-wrap', this.ddEl);

    show(this.itemsWrap);
    var size = getSize(this.valueEl);
    var iconOffset = this.options.withIcon ? 20 : 0;
    var marginTop = 0;
    if (openUp) {
      var itemsWrapSize = getSize(this.itemsWrap);
      marginTop = -itemsWrapSize[1] + 7;
    } else {
      marginTop = -size[1] - 7;
    }

    setStyle(this.itemsWrap, { 'marginTop': marginTop, 'marginLeft': (this.options.rtl ? (-this.options.width + size[0] + 3) : -11 - iconOffset) });

    this._initHoverEvent();
    this._initOutEvent();
    this._initKeypressEvent();

    MDropdown._currMDD = this;

    if (this.selectedItem) {
      this.selectValue(this.selectedItem[0], true);
    }

    var itemsNodes = itemsContentEl.childNodes, scrollTop = 0;
    var itemsWrapEl = geByClass1('mdd-items-wrap', this.ddEl);
    for (var i = 0; i < itemsNodes.length; i ++) {
      if (hasClass(itemsNodes[i], 'mdd-selected')) {
        itemsWrapEl.scrollTop = scrollTop - getSize(itemsWrapEl)[1] / 2 + getSize(itemsNodes[i])[1];
        break;
      }
      scrollTop += getSize(itemsNodes[i])[1];
    }

    setStyle(this.ddEl, {opacity: 1});

    addEvent(itemsWrapEl, 'wheel', this._onWheel.bind(this));

    this.options.onShow && this.options.onShow(this.ddEl);

    cancelEvent(event);
  }

  MDropdown.prototype.hide = function() {
    removeEvent(window, 'click', MDropdown._window_onOuterClick);
    removeEvent(window, 'keypress', MDropdown._window_onKeyPress);
    removeEvent(window, 'keydown', MDropdown._window_onKeyDown);
    hide(this.itemsWrap);
    MDropdown._currMDD = null;

    setStyle(this.ddEl, {opacity: ''});
    this.options.onHide && this.options.onHide(this.ddEl);

    cancelEvent(event);
  }

  MDropdown.prototype._getItemById = function(id) {
    for (var i = 0; i < this.items.length; i++) {
  ...
  MDropdown.prototype._initHoverEvent = function() {
    var itemsContentEl = geByClass1('mdd-items-content', this.itemsWrap);
    var itemsNodes = itemsContentEl.childNodes;
    var self = this;
    for (var i = 0; i < itemsNodes.length; i ++) {
      addEvent(itemsNodes[i], 'mouseenter', function(event) {
        if (self._preventMouseHover) return;
        var highlighted = geByClass1('hovered', itemsContentEl);
        if (highlighted) {
          removeClass(highlighted, 'hovered');
        }
        addClass(event.target, 'hovered');

        var item = self._getItemById(event.target.id);

        if (self._currCascadeMenuId && self._currCascadeMenuId != event.target.id) {
          self.itemsCascadeWrap.innerHTML = '';
          hide(self.itemsCascadeWrap);
          removeClass(geByClass1('cascade-open', self.itemsWrap), 'cascade-open');
        }

        /*
        if (isArray(item[2])) {
          self._currCascadeMenuId = event.target.id;
          var cascadeMenu = item[2];
          var cascadeMenuEl = '<div class="mdd-cascade" id="' + event.target.id + '">';
          var tmplFunc = self.options['cascadeMenuTemplate_' + event.target.id];
          each(cascadeMenu, function(i, m) {
            var itemInnerHtml = m[1];
            if (tmplFunc) {
              itemInnerHtml = tmplFunc(m);
            }
            cascadeMenuEl += '<div class="mdd-cascade-item" id="' + m[0] + '">' + itemInnerHtml + '</div>';
          });
          cascadeMenuEl += '</div>';

          self.itemsCascadeWrap.innerHTML = cascadeMenuEl;
          show(self.itemsCascadeWrap);

          var itemPos = getXY(event.target);
          var wrapPos = getXY(self.itemsWrap);
          var itemSize = getSize(event.target);

          setStyle(self.itemsCascadeWrap, { left: itemSize[0], top: itemPos[1] - wrapPos[1] });

          addClass(event.target, 'cascade-open');
        }*/
      });
      addEvent(itemsNodes[i], 'mouseleave', function(event) {
        if (self._preventMouseHover) return;
        removeClass(event.target, 'hovered');
      });
    }
  }

  MDropdown.prototype._highlightItem = function(itemId) {
    var itemsContentEl = geByClass1('mdd-items-content', this.itemsWrap);
    var highlighted = geByClass1('hovered', itemsContentEl);
    if (highlighted) {
      removeClass(highlighted, 'hovered');
    ...
    var itemsNodes = itemsContentEl.childNodes, scrollTop = 0;
    for (var i = 0; i < itemsNodes.length; i ++) {
      if (itemsNodes[i].getAttribute('id') == itemId) {
        this._preventMouseHover = true;

        clearTimeout(MDropdown._timer);
        MDropdown._timer = setTimeout(function() {
          self._preventMouseHover = false;
        }, 500);

        addClass(itemsNodes[i], 'hovered');
        itemsWrapEl.scrollTop = scrollTop - getSize(itemsWrapEl)[1] / 2 + getSize(itemsNodes[i])[1];
        break;
      }
      scrollTop += getSize(itemsNodes[i])[1];
  ...
  MDropdown.prototype._onWheel = function(event) {
    var itemsWrapEl = geByClass1('mdd-items-wrap', this.ddEl);

    var itemsContentHeight = getSize(geByClass1('mdd-items-content', this.ddEl))[1];
    var itemsWrapElHeight = getSize(itemsWrapEl)[1];

    if (itemsContentHeight != itemsWrapElHeight) {
      if (itemsWrapEl.scrollTop == 0 && event.deltaY < 0) {
        cancelEvent(event);
      } else if (itemsWrapEl.scrollTop == (itemsContentHeight - itemsWrapElHeight) && event.deltaY > 0) {
        cancelEvent(event);
      }
    }
  }

  MDropdown.prototype._onKeyDown = function(event) {
  ...
  MDropdown.prototype._onKeyPress = function(event) {
    this._onKey(event.which, event);
  }

  MDropdown.prototype._onKey = function(code, event) {
    var itemsContentEl = geByClass1('mdd-items-content', this.itemsWrap);

    if (code == 38 || code == 40) {
      var hovered = geByClass1('hovered', itemsContentEl);
      if (hovered) {
        var id = hovered.getAttribute('id');
        for (var i = 0; i < this.items.length; i++) {
          if (this.items[i][0] == id) {
            var index = code == 38 ? Math.max(i - 1, 0) : Math.min(i + 1, this.items.length - 1);
            this._highlightItem(this.items[index][0]);
            cancelEvent(event);
            break;
          }
        }
      ...
      var hovered = geByClass1('hovered', itemsContentEl);
      if (hovered) {
        this.selectValue(hovered.getAttribute('id'));
        this.hide();
      }
    } else if (code == 27) {
      this.hide();
    } else {
      this._doQuickSearch(code);
    }
  }

  MDropdown.prototype._doQuickSearch = function(code) {
    var self = this;
    ...
    if (code !== undefined) {
      ...
      if (self.currSearchStr.length == 1 && self.currSearchStr[0] == ch) {
    ...
    for (var i = this.currQuickSearchIndex; i < items.length; i++) {
      ...
      if (this.currSearchStr.length == 1 && this.currSearchStr == inner[0] ||
        this.currSearchStr.length > 1 && inner.indexOf(this.currSearchStr) >= 0) {
  ...
  MDropdown.prototype._initKeypressEvent = function() {
    addEvent(window, 'keypress', MDropdown._window_onKeyPress = this._onKeyPress.bind(this));
    addEvent(window, 'keydown', MDropdown._window_onKeyDown = this._onKeyDown.bind(this));
  }

  MDropdown.prototype._initOutEvent = function() {
    var self = this;
    function _onOuterClick(event) {
      ...
      while (limit-- && node != document) {
        if (!node) {
          toHide = true;
          break;
        } else if (matches(node, '.mdd-wrap')) {
          toHide = false;
          break;
        } else if (matches(node, '.mdd-item')) {
          removeEvent(window, 'click', this._onOuterClick);
          this.selectValue(node.id);
          toHide = true;
          break;
        } else if (matches(node, '.mdd-item-selected-wrap')) {
          toHide = true;
          break;
        }
    ...
    setTimeout(function() {
      addEvent(window, 'click', MDropdown._window_onOuterClick = _onOuterClick.bind(self));
    }, 1);
  }

  MDropdown.prototype.getValue = function() {
    return this.selectedItem;
  }

  MDropdown.prototype.selectValue = function(value, silent) {
    var selectedItem;
    ...
    if (!this.options.keepTitle) {
      this.valueEl.innerHTML = selectedItem[1];

      var selectedItemEl = geByClass1('mdd-item-selected', this.ddEl);
      if (selectedItemEl) {
        selectedItemEl.innerHTML = selectedItem[1];
        var itemsContentEl = geByClass1('mdd-items-content', this.itemsWrap);

        var itemsNodes = itemsContentEl.childNodes;
        for (var i = 0; i < itemsNodes.length; i ++) {
          removeClass(itemsNodes[i], 'mdd-selected');
          if (itemsNodes[i].id == value) {
            addClass(itemsNodes[i], 'mdd-selected');
          }
        }
      }
    }

    val(this.hiddenInput, value);

    this.selectedItem = selectedItem;

    if (!silent && this.options && this.options.onSelect) {
      try {
        this.options.onSelect(value, selectedItem);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  addEvent(this.ddEl, 'click', this._onClick.bind(this));

  this.selectValue(el.hasAttribute('data-value') ? el.getAttribute('data-value') : this.items[0][0], true);
}
...
function InlineDropdown(el, opts) {
  ...
  this._els = {
    ...
    hiddenInputEl: geByClass1('input', this._iddEl)
  }
...
InlineDropdown.prototype._rebuildDropdown = function() {
  ...
  function buildItem(item, addClass) {
    ...
    var checkHTML = '';
    if (item[2] == 1) {
      checkHTML = '<div class="idd_check_icon idd_checked"></div>';
    }
    if (item[2] == 0) {
      checkHTML = '<div class="idd_check_icon"></div>';
    }

    var noIcon = !!checkHTML;

    var sublist = _this._opts.sublists ? _this._opts.sublists[item[0]] : false;
    var sublistItemCls = sublist ? 'idd_sublist_parent' : '';
    var itemHTML =    '<div class="idd_item ' + addClass + ' ' + iconCls + ' ' + sublistItemCls + '" id="idd_item_' + item[0] + '" data-id="' + item[0] + '">' +
                        (_this._opts.withIcon && !noIcon ? ('<div class="idd_item_icon" id="' + item[0] + '"></div>') : '') +
                        checkHTML +
                        '<div class="idd_item_name">' + item[1] + '</div>' +
                      '</div>';
    return itemHTML;
  }
  ...
  each(this._items, function(i, item) {
    ...
    if (sublist) {
      each(sublist, function(i, sublistItem) {
        itemsHTML += buildItem(sublistItem, 'idd_sublist_item');
      });
    }
  });
...
InlineDropdown.prototype._onClick = function(event) {
  if (isVisible(this._els.popupEl)) return;
  ...
  var popupEl = this._els.popup = se('<div class="idd_popup"></div>');

  var openToUp = isOpenToUp();
  if (openToUp) {
  ...
  geByTag1('body').appendChild(popupEl);

  // hide sublist
  each(geByClass('idd_sublist_item'), function(i, el) {
    hide(el);
  });
  //
  ...
  setStyle(popupEl, {
    left: inlinePos[0] - additionalLeftOffset - 9 - 1, // .idd_header left-padding & border
    top: inlinePos[1] + addTop - 7 - 1, // .idd_header top-padding & border
    width: popupSize[0] + additionalWidth,
    opacity: 1
  });
...
InlineDropdown.prototype._highlightItem = function(itemId) {
  ...
  var currHighlighted = geByClass1('idd_hl', this._els.popup);
  if (currHighlighted) {
...
InlineDropdown.prototype._hoverItem = function(itemEl, andScroll) {
  this._unhoverItem();

  addClass(itemEl, 'idd_hover');
  this._hoveredItem = itemEl;

  if (hasClass(itemEl, 'idd_sublist_parent')) {
    each(geByClass('idd_sublist_item'), function(i, el) {
      show(el);
    });
  }

  // if sublist item is hovered, hover also parent item
  if (hasClass(itemEl, 'idd_sublist_item')) {
    var parentEl = itemEl.previousSibling;
    while(parentEl && !hasClass(parentEl, 'idd_sublist_parent')) {
      parentEl = parentEl.previousSibling
    }
    if (parentEl) {
      addClass(parentEl, 'idd_hover');
      this._hoveredParentItem = parentEl;
    }
  }
...
InlineDropdown.prototype._unhoverItem = function() {
...
InlineDropdown.prototype._quickSearch = function(code) {
  ...
  each(this._opts.sublists, function(id, sublist) {
...
InlineDropdown.prototype._onOuterClick = function(event) {
  ...
  while(node && node != document) {
    if (hasClass(node, 'idd_popup')) {
      needHide = false;
      break;
    } else if (node == this._els.popupHeader) {
      needHide = true;
      break;
    }
    node = node.parentNode;
...
InlineDropdown.prototype.select = function(id, silent) {
  ...
  // if no item found in main list, search in subists
  if (selected === false) {
    each(this._opts.sublists || {}, function(itemId, sublist) {
  ...
  if (hasClass(ge('idd_item_' + id), 'idd_sublist_parent')) {
    return;
  }

  if (selected.length > 2 && (selected[2] == 0 || selected[2] == 1)) { // if checked item
    var checkIcon = geByClass1('idd_check_icon', ge('idd_item_' + id));
    toggleClass(checkIcon, 'idd_checked');
  } else {
    // save it
