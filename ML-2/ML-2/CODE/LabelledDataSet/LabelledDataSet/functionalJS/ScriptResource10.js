...
AjaxControlToolkit.ModalPopupBehavior.prototype = {
    ...
    _attachPopup : function() {
        ...
        if (this._dragHandleElement && !this._dragBehavior) {
            this._dragBehavior = $create(AjaxControlToolkit.FloatingBehavior, {
                "handle" : this._dragHandleElement
        ...
        for (var j = 0; j < this._tagWithTabIndex.length; j++) {
            ...
            for (var k = 0 ; k < tagElements.length; k++) {
                if (Array.indexOf(tagElementsInPopUp, tagElements[k]) == - 1) {
                    this._saveTabIndexes[i] = {
                        tag: tagElements[k],
                        index: tagElements[k].tabIndex
                    };
        ...
        if ((Sys.Browser.agent === Sys.Browser.InternetExplorer) && (Sys.Browser.version < 7)) {
            ...
            for (var k = 0 ; k < tagElements.length; k++) {
                if (Array.indexOf(tagSelectInPopUp, tagElements[k]) == - 1) {
                    this._saveDesableSelect[i] = {
                        tag: tagElements[k],
                        visib: $common.getCurrentStyle(tagElements[k], 'visibility')
                    };
