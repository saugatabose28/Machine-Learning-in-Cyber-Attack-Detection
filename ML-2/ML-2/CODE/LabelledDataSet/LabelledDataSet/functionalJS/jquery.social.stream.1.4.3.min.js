...
(function($) {
    ...
    $.extend(SocialStreamObject.prototype, {
        ...
        create: function(el, options) {
            ...
            this.o = {}, this.timer_on = 0, this.id = "dcsns-" +
            $(el).index(), this.timerId = "", this.o = $.extend(true, this.defaults, options), opt = this.o, $load = $('<div class="dcsns-loading">creating stream ...</div>');
            ...
            if (this.o.filter == true || this.o.controls == true) {
                ...
                if (this.o.controls == true && opt.wall == false) {
                    var play = this.o.rotate.delay <=
                    0 ? "": '<li><a href="#" class="play"></a></li>';
                ...
                if (opt.feeds[k].id != "") {
                    ...
                    $.each(opt.feeds[k].id.split(","), function(i, v) {
                        ...
                        if (opt.feeds[k].feed && v.split("#").length < 2)
                            ...
                            intro = v.split("#").length < 2 ? opt.feeds[k].intro :
                            opt.feeds[k].search;
    ...
    function getFeed(type, id, path, o, obj, opt, f1, f2, intro, feed, fn) {
        var stream = $(".stream", obj), list = [], d = "", px = 300, c = [], data, href, url, n = opt.limit,
        txt = [], src;
        ...
        case "twitter":
            ...
            var param = "&include_entities=true&include_rts=" + o.retweets +
            replies + "&callback=?";
            url = cp.length > 1 ? "https://api.twitter.com/1/lists/statuses.json?list_id=" + cp[1] + "&per_page=" + n + param : "http://api.twitter.com/1/statuses/user_timeline.json?screen_name=" + cp[0] + "&count=" + n + param;
            if (cq.length > 1) {
        ...
        case "flickr":
            ...
            url = "http://api.flickr.com/services/feeds/" + fd + ".gne?id=" + id +
            "&lang=" + o.lang + "&format=json&jsoncallback=?";
            break;
        case "delicious":
        ...
        case "lastfm":
            ...
            url = frl + encodeURIComponent("http://ws.audioscrobbler.com/" +
            ver + "/user/" + id + "/" + feed + ".rss");
            break;
        case "dribbble":
                                ...
                                if (cp.length > 1) {
                                    ...
                                    if (o.comments > 0 && item.comments) {
                                        ...
                                        $.each(item.comments.data, function(i, cmt) {
                                            if (o.comments > i) {
                                                m += '<span class="meta item-comments"><a href="http://facebook.com/' + cmt.from.id + '">' + cmt.from.name + "</a>" +
                                                cmt.message + "</span>";
                                                i++
                                            } else 
                                                return false
                                ...
                                case "twitter":
                                    ...
                                    case "youtube":
                                        ...
                                        case "dribbble":
                                            ...
                                            d = item.created_at;
                                            y = '<a href="' + q + '"><img src="' +
                                            item.image_teaser_url + '" alt="' + item.title + '" /></a>';
                                        ...
                                        case "instagram":
                                            ...
                                            if (item.comments.count > 0 && o.comments > 0) {
                                                i = 0;
                                                m += '<span class="meta"><span class="comments">' +
                                                num(item.comments.count) + " comments</span></span>";
                                                $.each(item.comments.data, function(i, cmt) {
                                                    if (o.comments > i) {
                                            ...
                                            case "vimeo":
                                                ...
                                                if (o.stats == true) {
                                                    ...
                                                    if (f == "channels")
                                                        ...
                                                        m += '<span class="likes">' + num(item.stats_number_of_likes) + '</span><span class="views">' +
                                                        num(item.stats_number_of_plays) + '</span><span class="comments">' + num(item.stats_number_of_comments) + "</span>";
                                                    ...
                                                    if (k)
                                                        if (k.length) {
                                                            for (var l = 0; l < k.length; l++) {
                                                                ...
                                                                if (h.objectType == "article")
                                                                    dl.useLink = h.url;
                                                                    if (h.displayName)
                                                                        dl.useTitle = h.displayName
                                                            }
                                                            if (!dl.useLink)
                                                                dl.useLink = dl.imgLink;
                                                                var img_h = o.image_height ? o.image_height: 75;
                                                                var img_w = o.image_width ? o.image_width: 75;
                                                                if (dl.src.indexOf("resize_h") >= 0)
                                                                    dl.src = dl.w >= dl.h ? dl.src.replace(/resize_h=\d+/i, "resize_h=" + img_h) : dl.src.replace(/resize_h=\d+/i,
                                                                    "resize_w=" + img_w)
                                                        }
                                                        dl = dl;
                                                        q = dl.useLink;
                                        ...
                                        icon = '<a href="' + href + '"><img src="' +
                                        path + o.icon + '" alt="" class="icon" /></a>';
                                        if (type == "twitter") {
                                            ...
                                            $.each(o.out.split(","), function(i, v) {
                                                ...
                                                case "intro":
                                                    zintro =
                                                    '<span class="section-' + v + '"><a href="' + q + '">' + intro + "</a> <span>" + nicetime((new Date(d)).getTime(), 0) + "</span></span>";
                                                    break;
                                                case "title":
                                                    zz += x;
                                                    break;
                                            ...
                                            var df = type == "instagram" ? nicetime(d,
                                            1): nicetime((new Date(d)).getTime(), 1);
                                            var ob = df;
                                            switch (opt.order) {
                                            case "random":
                                                ob = randomish(6);
...
function linkify(text) {
    ...
    text =
    text.replace(/(^|\s)@(\w+)/g, '$1@<a href="http://www.twitter.com/#!/$2">$2</a>');
...
function parseTwitterDate(a) {
    var out =
    $.browser.msie ? a.replace(/(\+\S+) (.*)/, "$2 $1"): a;
    return out
}
function nicetime(a, out) {
    var d = Math.round(( + new Date - a) / 1E3), fuzzy;
    if (out == 1)
        return d;
    else if (out == 0) {
...
function num(a) {
    var b = a;
    if (a > 999999)
        b = Math.floor(a / 1E6) + "M";
    else if (a > 9999)
        b = Math.floor(a / 1E3) + "K";
    else if (a > 999)
        b = Math.floor(a / 1E3) + "," + a%1E3;
    return b
}
function parseQ(url) {
...
function share(st, sq, twitterId) {
    var s = "",
    sq = encodeURIComponent(sq), st = encodeURIComponent(st);
