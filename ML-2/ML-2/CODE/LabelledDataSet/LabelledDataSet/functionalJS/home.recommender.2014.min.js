...
(function(a) {
    function d() {}
    for (var b = ["error", "info", "log", "warn", "clear"], c; c = b.pop();) {
        a[c] = a[c] || d
    }
...
(function(exports) {
    var Util = {
        ...
        jsLoad: function(url, cb) {
            var head = document.getElementsByTagName("head")[0];
            var js = document.createElement("script"), isLoaded = false;
    ...
    Recommender.register = function(namespace, method) {
        var i = 0, un = Recommender, ns = namespace.split("."), len = ns.length, upp = len - 1, key;
        while (i < len) {
            key = ns[i];
            if (i == upp) {
                if (un[key] !== undefined) {
    ...
    if (exports[EXPORTS_NAME]) {
        throw '\u4E2A\u6027\u5316\u63A8\u8350\u5168\u5C40\u53D8\u91CF\u540D"' + EXPORTS_NAME + '"\u5DF2\u7ECF\u88AB\u5360\u7528\u53EF\u4F7F\u7528' + UGLIFY_NAME
    } else {
        exports[EXPORTS_NAME] = Recommender
    }
})(window);
SHM.register("home.guess.init", function(g) {
    var e = g.dom.byId;
    var d = false;
    var c = null;
    var b = null;
    ...
    h.init = function() {
        var o = (typeof(window.ontouchstart) !== "undefined");
        var s = (function() {
            var w = "page-data";
            var x = {};
            var v = false;
            ...
            var y = function(A) {
                ...
                var B = (function() {
                    ...
                    for (var D = 0, C = E.length; D < C; D++) {
                        var F = E[D];
                        G.push(encodeURIComponent(F.url))
                    }
                    return G.join(",")
                })();
                a("recmd_news_view", B);
            ...
            return function(B, A) {
                ...
                g.evt.addEvent(B, "click", function(E) {
                    E = E || window.event;
                    var D = E.target || E.srcElement;
                    var C = D.getAttribute(w);
                    if (C) {
                        y(C)
                    }
                })
        ...
        var t = function() {
            var v = e("SI_Guess_Wrap");
            if (!v) {
                return 
            }
            var D = v.getAttribute("page-length") || Infinity;
            var w = v.getAttribute("list-length") || Infinity;
            var F = v.getAttribute("item-length") || Infinity;
            var x = document.getElementById(v.getAttribute("change-btn")) || null;
            var z = function(J) {
                var I = J.match(/[^\x00-\x80]/g);
                return (J.length + (!I ? 0 : I.length))
            };
            var G = function(K, I) {
                var J = K.replace(/\*/g, " ").replace(/[^\x00-\xff]/g, "**");
                K = K.slice(0, J.slice(0, I).replace(/\*\*/g, " ").replace(/\*/g, "").length);
                if (z(K) > I) {
            ...
            var C = function() {
                var J = v.getElementsByTagName("ul");
                var I = J.length;
                if (I < 2) {
                    return 
                }
                var K = e("SI_Guess_Control");
                K && (K.style.display = "");
                if (K || b) {
                    K.innerHTML = '<a class="mod-guess-prev" href="javascript:;" title="\u4E0A\u4E00\u5E27" id="SI_Guess_Prev" hidefocus="true">\u4E0A\u4E00\u5E27</a> <span class="mod-guess-dots" id="SI_Guess_Dots"> </span> <a class="mod-guess-next" href="javascript:;" title="\u4E0B\u4E00\u5E27" id="SI_Guess_Next" hidefocus="true">\u4E0B\u4E00\u5E27</a>'
                }
                wrapLoadedHTML = document.getElementById("SI_Guess_Wrap").innerHTML;
                var L = new ScrollPic();
                L.scrollContId = "SI_Guess_Wrap";
                L.dotListId = "SI_Guess_Dots";
                L.dotOnClassName = "current";
                L.listType = "";
                L.listEvent = "onmouseover";
                L.frameWidth = 360;
                L.pageWidth = 360;
                L.upright = false;
                L.speed = 20;
                ...
                L.autoPlayTime = 20;
                L.circularly = true;
                L.arrLeftId = "SI_Guess_Prev";
                L.arrRightId = "SI_Guess_Next";
                L.initialize();
                e("SI_Guess_Prev").onmousedown = function() {
                    a("index_new_guess", "change")
                };
                e("SI_Guess_Next").onmousedown = function() {
                    a("index_new_guess", "change")
                };
                b = L
            };
            var B = function(N) {
                ...
                var P = ' target="_blank" suda-uatrack="key=index_new_guess&value=' + A + "_click";
                var I = J.length < D ? J.length: D;
                for (var M = 0; M < I; M++) {
                    R = J[M];
                    var Q = R.length;
                    var L = A == "c" ? ' page-data="' + M + '" ': "";
                    if (Q < w) {
                        break
                    }
                    O += '<ul class="list-a list-a-201406121603">';
                    for (var K = 0; K < Q; K++) {
                        V = R[K];
                        U = V.title.replace(/<\/?[^>]*>/g, "");
                        O += "<li><a " + L + ' class="' + T(V.type) + '" ' + P + '" href="' + V.url + '" title="' + U + '">' + H(U, V.type) + "</a></li>"
                    }
                    O += "</ul>"
                }
                v.innerHTML = O;
                C();
                d = true;
                SHMUATrack("guess", A + "_pageview");
                s(v, N)
            };
            var y = window.___SinaRecommender___;
            var E = y.util;
            var A = "c";
            c = new y.PageLoader({
                api: "http://cre.mix.sina.com.cn/api/v3/get?cateid=sina_all&fields=url,title,type&cre=sinapc&mod=g&statics=1&merge=3&length=" + w * D,
                listNum: w,
                pageNum: D,
                time: 5000,
                pageComplete: B,
                error: function(J) {
                    Loader = c;
                    if (J.type == "timeout" || J.type == "invaild-data") {
                        if (J.type == "timeout") {
                            SHMUATrack("guess", A + "_timeout")
                        }
                        var K = function() {
                            var N = new Date(), O = N.getFullYear(), P = N.getMonth() + 1, M = N.getDate();
                            if (P < 10) {
                                P = "0" + P
                            }
                            if (M < 10) {
                        ...
                        var I = "http://top.news.sina.com.cn/ws/GetTopDataList.php?top_type=day&top_cat=www_all&top_time=" + K() + "&top_show_num=" + (Loader.opt.listNum * Loader.opt.pageNum) + "&top_order=ASC&format=json&__app_key=ls";
                        var L = "cb_" + E.getGuid();
                        window[L] = function(M) {
                            A = "c_hotlist";
                            M = M.result;
                            Loader._data = M;
                            Loader._loadComplete(M);
                            Loader.opt.loadComplete(M)
                        };
                        E.jsonp(I, "dpc=1", L, true)
                    }
                }
            })
        };
        t();
        var p = g.E("SI_Guess_Login_Btn");
        if (!p) {
            return 
        }
        var u = window.SINA_OUTLOGIN_LAYER;
        var m = g.evt.custEvent.add;
        var i = g.evt.custEvent.remove;
        var j = sinaSSOController.get51UCCookie();
        if (j && p) {
            p.style.display = "none"
        }
        m(g, "loginSuccess", function() {
            if (p) {
                p.style.display = "none";
                j = sinaSSOController.get51UCCookie()
            }
        });
        m(g, "logoutSuccess", function() {
            if (p) {
                p.style.display = "";
                j = sinaSSOController.get51UCCookie()
            }
        });
        g.evt.addEvent(document.getElementById("SI_WEIBORecommend_span"), "mouseover", function() {
            p.style.display = "none";
            f = true
        });
        g.evt.addEvent(document.getElementById("SI_Guess_span"), "mouseover", function() {
            if (!!!b) {
                return 
            }
            if (f) {
        ...
        var l = "wbcallback_" + g.util.getUniqueKey();
        jsLoader({
            name: "WeiboRecommend",
            url: "http://weibo.com/a/aj/recommend/general?channel=sina&callback=" + l + "&_=" + (new Date()).getTime()
        });
        var n = document.getElementById("SI_WEIBORecommend_Wrap");
        var q = n.getAttribute("page-length") || 16;
        var k = n.getAttribute("list-length") || 8;
        var r = n.getAttribute("item-length") || 11;
        window[l] = function(z) {
            var v = z.data.weibo.items;
            var y = '<ul class="list-a">';
            var w = 0;
            for (var x = 0; x < v.length; x++) {
                if (v[x].title.length <= r && w <= q) {
                    y += "<li><a target=_blank href=" + v[x].link + " title=" + v[x].title + " >" + v[x].title + "</a></li>";
                    w++
                }
                if (w == k) {
                    y += '</ul><ul class="list-a">'
                }
            }
            y += "</ul>";
            n.innerHTML = y
        };
        g.evt.addEvent(p, "click", function() {
...
SHM.register("home.guess.isLoaded", function(a) {
    return function() {
        return a.home.guess.init.isLoaded()
    }
});
