	...
	var app = {
		...
		render1:function(list,wrap){
			...
			var renderBLive = function(item,tomorrow){
				...
				return '<a href="'+newUrl+'" target="_blank" title="'+item.live_name+'" class="item '+tomorrow.clz+' blive"> <div class="txt">'+iTag+'<span class="tip">正在播出</span> <span class="team">'+item.live_short_name+'</span> </div>'+bg+'</a>';
			};
			var renderHot = function(item,tomorrow){
				return '<a href="'+newUrl+'" target="_blank" title="'+item.live_name+'" class="item '+tomorrow.clz+' hot"> <div class="txt">'+iTag+'<span class="tip">欢迎收看 '+tomorrow.txt+' '+hourMin+'</span> <span class="team">'+item.live_short_name+'</span> </div>'+bg+'</a>';
			};
			...
			// 未来的比赛包括正大直播的链接统一 newUrl http://sports.sina.com.cn/tv，除了带小live图标的比赛外 renderSItem  具体@赵庆
