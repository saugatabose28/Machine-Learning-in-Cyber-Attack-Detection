    ...
    function k(l, m) {
        ...
        i[n] = setTimeout(function() {
            ...
            if (!l.offsetHeight) {
                ...
                while (o && o !== p) {
                    if (!o.style || g.hasClass(o, 'hidden_elem') || o.style.display === 'none' || o.style.height === '0px' || o.style.height === 0 || o.style.height === '0' || o.childNodes.length === 0)
                        return;
                    o = o.parentNode;
                }
                if (o === p)
...
__d("Visibility", ["mixInEventEmitter"], function(a, b, c, d, e, f, g) {
    ...
    if (document.addEventListener && i)
        document.addEventListener(i, function l() {
...
__d("ViewableImpressionHeatmapLogger", ["Banzai"], function(a, b, c, d, e, f, g) {
    ...
    h.prototype.onTrackingCompleted = function(i) {
        ...
        if (this.$ViewableImpressionHeatmapLogger0())
            this.$ViewableImpressionHeatmapLogger1(i, j);
        this.logToServer(j);
...
__d("ViewableImpressionTracker", ["Banzai", "copyProperties", "CSSCore", "cx", "DOM", "getElementPosition", "getViewportDimensions", "mixInEventEmitter", "Style", "URI"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {
    function q(r, s, t) {
        "use strict";
        this.id = r;
        this.element = s;
    ...
    q.prototype.getPercentInViewport = function() {
        "use strict";
        var r = m(), s = l(this.element);
        if (s.x < r.width && s.x + s.width > 0 && s.y < r.height && s.y + s.height > 0 && o.get(this.element, 'visibility') !== 'hidden' && o.get(this.element, 'opacity') !== '0') {
            var t = Math.max(s.x, 0), u = Math.min(s.x + s.width, r.width), v = Math.max(s.y, 0), w = Math.min(s.y + s.height, r.height);
            if ((s.height * s.width) === 0)
                return 100;
            if (this.config.require_horizontally_onscreen && ((u - t) < s.width || i.hasClass(this.element, "desktop_hscroll_offscreen")))
                return 0;
    ...
    q.prototype.onVisible = function() {
        ...
        if (this.isLogged()) {
            ...
            this.$ViewableImpressionTracker2();
        if (!this.waitForLogged&&!this.isLogged()) {
            this.waitForLogged = true;
            this.viewableTimeout = setTimeout(function() {
                this.waitForLogged = false;
                var t = this.getPercentInViewport(), u = t > this.config.pixel_in_percentage;
                if (!u) {
    ...
    q.prototype.onHidden = function() {
        ...
        } else if (!this.config.log_initial_nonviewable&&!this.isLogged() && this.isVisible)
            this.logNonViewableImpression();
        this.isVisible = false;
    ...
    q.prototype.logImpression = function(r, s) {
        ...
        if (this.isLogged())
            return;
        var t = this.getToken(), u = Math.floor(Date.now() / 1000), v = {
            xt: t,
            isv: r,
            cts: u
        };
    ...
    q.prototype.logWithIFrame = function(r) {
        "use strict";
        this.iframe = k.create('iframe', {
            src: new p(this.config.impressionURL).addQueryData(r),
            width: 0,
            height: 0,
            frameborder: 0,
            scrolling: 'no',
        ...
        k.appendContent(this.element, this.iframe);
    };
    q.prototype.$ViewableImpressionTracker4 = function() {
        "use strict";
        return 0;
    };
    q.prototype.$ViewableImpressionTracker0 = function() {
        "use strict";
        if (this.$ViewableImpressionTracker4()) {
            o.set(this.element, 'background-color', '#abab9a');
            o.set(this.element, 'outline', '3px solid #abab9a');
        }
    };
    q.prototype.$ViewableImpressionTracker2 = function() {
        "use strict";
        if (this.$ViewableImpressionTracker4()) {
            o.set(this.element, 'background-color', '#e4f70a');
            o.set(this.element, 'outline', '3px solid #e4f70a');
        }
    };
    q.prototype.$ViewableImpressionTracker3 = function() {
        "use strict";
        if (this.$ViewableImpressionTracker4()) {
            o.set(this.element, 'background-color', null);
            o.set(this.element, 'outline', null);
        }
    };
    q.prototype.$ViewableImpressionTracker1 = function() {
        "use strict";
        if (this.$ViewableImpressionTracker4()) {
            o.set(this.element, 'background-color', '#047515');
            o.set(this.element, 'outline', '3px solid #047515');
        }
    };
    n(q, {
        visible: true,
...
__d("VisibilityTracking", ["Banzai", "ErrorUtils", "ScriptPath", "SubscriptionsHandler", "Visibility", "VisibilityTrackingHelper", "collectDataAttributes", "copyProperties", "getElementPosition", "pageID", "throttle"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q) {
    var r = 'visibility_tracking', s = '[data-vistracking]', t = 500, u = 50, v = 50, w = 50, x = 50, y = {
        VISIBLE: 'visible',
        HIDDEN: 'hidden'
    }, z = {
        DEFAULT: 'default',
    ...
    function ea(fa) {
        "use strict";
        fa = fa || {};
        if (!fa.bypass_banzai_check&&!g.isEnabled(r))
            return;
        ...
        if (!this.skipVisibilityHiddenEvents)
            k.addListener(k.HIDDEN, h.guard(this.clearAllVisibleElements, 'VisibilityTracking:hidden', this));
        ...
        if (this.extraInit)
            h.applyWithGuard(this.extraInit.bind(this));
    }
    ...
    ea.prototype.fireEvent = function() {
        ...
        for (var ga in fa) {
            ...
            } else if (ja && (!(ga in this.visibleElementInfo) || this.handleAllVisibleEvents))
                this.handleEvent(ha, aa);
        }
    ...
    ea.prototype.onUnload = function() {
        ...
        if (this.extraCleanup)
            h.applyWithGuard(this.extraCleanup.bind(this));
    };
    ea.init = function(fa) {
        "use strict";
        new ea(fa);
...
__d("ViewableImpressionEventHandler", ["copyProperties", "ViewableImpressionHeatmapLogger", "ViewableImpressionTracker", "VisibilityTracking"], function(a, b, c, d, e, f, g, h, i, j) {
    ...
    n.prototype.getImpressionTracking = function(o) {
        ...
        if (!r) {
            ...
            if (this.doHeatmapLogging)
                h.logFromViewableImpressionTracker(r, this.heatmapLoggingDurationMS);
        }
        return r;
    };
    ...
    n.prototype.handleEvent = function(o, event) {
        ...
        if (event.name === j.EVENT.VISIBLE) {
            p.onVisible();
            if (!this.visibleElementInfo[p.getID()])
                this.visibleElementInfo[p.getID()] = {
                    elem: o
                };
...
__d("EgoUnitSlideInsert", ["Animation", "CSS", "DataStore", "DOM", "Ease", "Event", "Parent", "TidyArbiterMixin", "cx", "copyProperties", "tidyEvent"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q) {
    var r = 'sliding', s = "EgoSlider/End", t = p({
        isSliding: function(u) {
            return i.get(u, r);
        },
    ...
    var q = {
        ...
        clearHeader: function() {
            ...
            for (var t = 0; t < r.length; ++t)
                s = s.concat(k.scry(r[t], '.uiHeader'));
            for (t = 0; t < s.length; ++t) {
                ...
                if (!u || u.childNodes.length === 0) {
                    ...
                    if (j.hasClass(v, 'ego_appended_units') && v.childNodes.length === 0)
                        k.remove(s[t]);
                }
            }
        }
