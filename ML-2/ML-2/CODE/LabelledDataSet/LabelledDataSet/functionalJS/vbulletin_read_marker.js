...
vB_AJAX_ReadMarker.prototype.mark_read = function() {
    YAHOO.util.Connect.asyncRequest("POST", fetch_ajax_url("ajax.php?do=markread&f=" + this.forumid), {
...
vB_AJAX_ReadMarker.prototype.update_thread_status = function(D) {
    ...
    if (C) {
        C.src = this.fetch_old_src(C.src, "thread")
    }
...
vB_AJAX_ReadMarker.prototype.fetch_old_src = function(B, A) {
    var C = B.replace(/_(new)([-_])(.+)$/i, (A == "thread" ? "$2$3" : "_old$2$3"));
    return C
};
...
function init_forum_readmarker_system() {
    ...
    for (var B = 0; B < A.length; B++) {
        if (A[B].id && A[B].id.substr(0, vB_ReadMarker.forum_statusicon_prefix.length) == vB_ReadMarker.forum_statusicon_prefix) {
            if (A[B].src.search(/\/([^\/]+)(new)(_lock)?-48\.([a-z0-9]+)$/i)!=-1) {
                img_alt_2_title(A[B]);
...
vB_AJAX_ForumReadMarker.prototype.mark_threads_read = function() {
    var D = YAHOO.util.Dom.getElementsByClassName("new", "li", "threadlist");
    for (var A = 0; A < D.length; A++) {
        YAHOO.util.Dom.removeClass(D[A], "new");
        var C = YAHOO.util.Dom.getElementsByClassName("threadtitle_unread", "a", D[A]);
        C = C[0];
        if (C) {
            YAHOO.util.Dom.removeClass(C, "threadtitle_unread");
            var E = YAHOO.util.Dom.getElementsByClassName("gotonewpost", "img", D[A]);
            if (E && E[0]) {
                var B = YAHOO.util.Dom.getAncestorByTagName(E[0], "a");
                if (B) {
...
function mark_forum_and_threads_read(A) {
    if (AJAX_Compatible) {
        vB_ReadMarker[A] = new vB_AJAX_ReadMarker(A);
        vB_ReadMarker[A].mark_read();
        vB_ReadMarker[A] = new vB_AJAX_ForumReadMarker();
        vB_ReadMarker[A].mark_threads_read()
    } else {
        window.location = "forumdisplay.php?" + SESSIONURL + "do=markread&forumid=" + A + "&markreadhash=" + SECURITYTOKEN
    }
    return false
