...
(function(a, b, c) {
    ...
    if (a.Modernizr)
        for (var j in i)
            Modernizr.hasOwnProperty(j) || Modernizr.addTest(j, i[j]);
    else 
    ...
    b.fn.isotope = function(a, c) {
        if (typeof a == "string") {
            ...
            this.each(function() {
                ...
                if (!c)
                    r("cannot call methods on isotope prior to initialization; attempted to call method '" + a + "'");
                else {
...
jQuery(window).load(function() {
    ...
    var $container = jQuery('.wall-outer .stream'), filters = {};

    jQuery('.wall-outer .filter a').click(function() {
        var $this = jQuery(this), isoFilters = [], prop, selector;
        jQuery('.filter a').removeClass('iso-active');
