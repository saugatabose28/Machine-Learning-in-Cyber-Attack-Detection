/*!
* PBS Bootstrap by @PBS
* Copyright 2012 PBS.org
...
!function($) {
    ...
    $(document).on("click.dropdown.data-api touchstart.dropdown.data-api", toggle, Dropdown.prototype.toggle)
}(window.jQuery);
!function($) {
    "use strict";
...
!function($) {
    "use strict";
    function moveStationLogo() {
        if ($(window).width() < 600) {
            return $("#station-logo").css("margin-left", - ($("#station-logo").width() / 2)).removeClass("pull-left")
        } else {
            return $("#station-logo").css("margin-left", 0).addClass("pull-left")
        }
    }
    function applySponsorBottomBorder() {
        var funders = $("div.funder-image");
        var borders = $("div.bordered");
        var cxWidth = $($("div.funders").get(0)).width();
        var summedWidths = 0;
        $.each(borders, function(i, b) {
            $(b).remove()
        });
        var rows = new Array;
        var k = 0, l = 0;
        ...
        for (var i = 0; i < funders.length; i++) {
            var fWidth = $(funders[i]).outerWidth(true);
            if (summedWidths + fWidth <= cxWidth) {
                l++;
                summedWidths += fWidth
            } else {
                ++k;
        ...
        if (rows.length > 1) {
            for (var i = 0; i < rows.length - 1; i++) {
                ...
                bordered.setAttribute("class", "clearfix bordered");
                $(bordered).insertAfter($(rows[i][j]))
            }
        }
    }
    function swapFundersWithCopyright() {
        var copyrightBox = $("#flexi-copyright");
        var footerLinks = $("#flexi-links");
        var fundersBox = $("#flexi-sponsors");
        if ($(window).width() <= 599) {
            $(copyrightBox).after($(fundersBox))
        }
    }
    function normalizePromoBoxes() {
        var promoCxs = $("div.promo-container");
        $.each(promoCxs, function(i, cx) {
            var max = 0;
            var promos = $(cx).find("div.promo");
            for (var j = 0; j < promos.length; j++) {
                $(promos[j]).css("height", "");
                if ($(promos[j]).outerHeight() > max) {
                    max = $(promos[j]).outerHeight()
                }
            }
            for (var j = 0; j < promos.length; j++) {
                $(promos[j]).height(max)
    ...
    $(window).resize(function() {
        moveStationLogo();
        swapFundersWithCopyright();
        applySponsorBottomBorder();
        if (/iPad/i.test(navigator.userAgent) || $.browser.msie) {
            normalizePromoBoxes()
        }
    });
    $(document).ready(function() {
        $(".row").each(function(i, item) {
            $(item).removeClass("row").addClass("row-fluid")
        });
        var plugins = ["#blog-article", ".sixteen.columns.section", ".newsletter", ".text-plugin", ".search-results", ".media-feed"];
        $.each(plugins, function(i, plugin) {
            $(plugin).addClass("box")
        });
        var clearfixPlugins = [".newsletter", ".form-newsletter"];
        $.each(clearfixPlugins, function(i, plugin) {
            $(plugin).addClass("clearfix")
        });
        swapFundersWithCopyright();
        setTimeout(function() {
            applySponsorBottomBorder()
